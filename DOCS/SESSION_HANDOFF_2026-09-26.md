...
3. **523, slice 3:** the reminder schedule the adviser controls (before the due date, on it, and at 7,
   14 and 30 days overdue, each a switch with editable wording; the nightly job as the engine, with
   send-once guards per invoice per stage and a preview); then slice 4. Slices 1 and 2 are live: the
   owner should glance at Dashboard, Client Invoices and "Who owes what", signed in, for the look.
   524 (dictation) waits for the owner's word.
