...
- Whether SendGrid event tracking is working at all (H-2). The webhook correctly rejects everything until a real public key is configured, and no key exists in source. SETTLED BY: checking the Email:SendGridEventWebhookPublicKey app setting on ipro-prod-web in Azure - if it is unset or still the placeholder, every open, click and bounce is being discarded right now.
