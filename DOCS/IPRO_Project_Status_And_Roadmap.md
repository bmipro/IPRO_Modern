...
- Existing already-cached rows in production have `RelatedEntityId = NULL` until the next nightly job run repopulates them (a migrated column can't retroactively know what an already-generated suggestion referred to) — so an agent who already has a stale suggestion showing won't see it clear until the next `ai-daily-digest` run, not immediately on deploy. This is a one-time transition gap, not an ongoing limitation.
