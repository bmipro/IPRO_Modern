...
- After the user's first live test, added: a post-vote redirect back to the agent's own published website; a fix for the Results link being wrongly hidden whenever a send didn't register a successful SendGrid delivery; and a new **Poll Results** website block agents can add to any page, showing live results for a chosen poll once it clears 10 responses (to protect anonymity). Also identified (fix in progress) that `App:BaseUrl` was never configured in Azure, so poll and newsletter links fall back to the raw `azurewebsites.net` hostname — binding a real `app.iproadvisers.com` domain to fix it, same as was done for the admin app.
