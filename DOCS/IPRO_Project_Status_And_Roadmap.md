...
### Branded newsletter wrapper (not built — designed 2026-07-22)

**The problem, confirmed by direct comparison**: today's newsletters ship with zero visual branding. `NewsLetterDispatcher.AppendUnsubscribeHtml` is the *only* transformation applied to `HtmlBody` before send — a plain string concat of the agent's rich-edited content plus an unsubscribe `<div>`. No header, no banner, no footer, no branded chrome of any kind. This was confirmed by reading the actual dispatch code, not assumed.

**Where the idea comes from**: the user shared a legacy version's newsletter templates (`index.htm` through `index6.htm`, a 2009-era set) and a screen recording of that legacy editor (`newsletterjuly22.mp4`). The rendered templates are genuinely well-composed — branded header banner, a colored title bar with edition name + site link, an "In this issue" summary box, styled article sections with "Read More" truncation, a sidebar with agent photo/services/CTA buttons, and a proper footer with full contact info + company logo. Watching the actual editor flow revealed the source of that polish: it came entirely from a **fixed wrapper template** (banner picked from a small stock gallery, plus a couple of style toggles) around **content that was authored more primitively than today** — a plain textarea, no rich formatting at all. In other words: the old system's content-authoring was worse than what IPRO has now (a real rich-text editor), but its presentation wrapper was strictly better. The fix is additive, not a rebuild — add the wrapper, leave the authoring experience alone.

**Design — composed at send time, not baked into stored content**, mirroring the existing "template renders content" philosophy already used by the website builder (`WebsiteContentBlock` → rendered inside a chosen `WebsiteTemplate`'s markup):
1. Add two nullable columns to the existing `NewsLetter` table (`EnsureTableColumnAsync`, both apps — remember the `db.Database.OpenConnectionAsync()`/`CloseConnectionAsync()` wrap that helper needs, see `09_TROUBLESHOOTING.md`): `BannerUrl` (string) and `Edition` (string, e.g. "November 2026 Newsletter"; defaults to `{month} {year} Newsletter` if left blank).
2. New shared static helper `NewsletterHtmlComposer.Wrap(NewsLetter, AgentUser)` in `IPRO.Business` (same placement pattern as `AiUsageRecorder`, since both `IPRO.Web`'s Preview action and `IPRO.Email`'s `NewsLetterDispatcher` need the identical output — Preview must show exactly what actually gets sent, not an approximation). Builds: banner `<img>` (if set), a colored title bar showing Edition + a link to the agent's site — using the agent's own `PortalAccentColor` (the same accent-picker color from the portal chrome) as the bar color, a nice cohesive touch that costs nothing extra — then the untouched `HtmlBody` content, then a footer with the agent's name/phone/email/company. `AppendUnsubscribeHtml` still runs after this, unchanged.
3. `NewsLetterDispatcher.DispatchSendAsync` and `DispatchDripStepAsync`: call `NewsletterHtmlComposer.Wrap(...)` before the existing `AppendUnsubscribeHtml(...)` call.
4. `NewsletterController`: banner picker in Create/Edit, reusing `WebsiteStarterBannerCatalog.All` (the exact same stock-image gallery already built for website Hero blocks — same grid markup/JS pattern, same image files) plus a new `Edition` text input. New `ApplyBanner(int newsletterId, string bannerUrl)` POST action mirroring `WebsitePagesController.ApplyImageToBlock`'s shape, writing to `NewsLetter.BannerUrl`.
5. `Newsletter/Preview` action calls the same `NewsletterHtmlComposer.Wrap` so what an agent previews is pixel-identical to what actually sends.

**Deliberately deferred, not phase 1**:
- The two-column layout with a sidebar (agent photo, service list, CTA buttons like "Schedule an Appointment"/"Get a Quote"). Old table-based multi-column email HTML is still technically the correct approach for cross-client compatibility (Gmail/Outlook/Apple Mail still have poor CSS support) — this isn't retro for its own sake — but a sidebar needs those destination pages/actions to actually exist and be configurable per agent, which is real additional scope. Phase 1 ships a single-column wrapper (header + content + footer): simpler, more reliable across clients, and mobile-friendly by default. The sidebar CTA block is a natural phase 2.
- The legacy "Header / Shadow / Show Title Name" style toggles — ship one well-designed wrapper first, add variants only if agents actually ask.
- Structured multi-article authoring (the legacy system's separate `Title` + `Displayed Characters` + `Content` per article, producing the "In this issue" list and "Read More" truncation automatically). A `NewsLetterArticle` entity already exists in the schema but isn't wired into the current Create/Edit flow or dispatcher — worth investigating separately why it's unused before deciding whether to build on it or replace it; not needed to fix the actual complaint (the wrapper), so kept out of phase 1 scope.
- Giving `NewsLetterTemplate` (the admin-curated starter-content templates) its own default `BannerUrl` so a template comes with a suggested banner pre-picked. Cheap to add later, not required for phase 1.
