...
- **Correction, days later, found by the user in real production use**: the local verification above proved the *login page* itself now loads on an agent's own domain, but never checked what happens *after* a successful sign-in. It's broken in a serious way: `AccountController`'s cookie auth has no explicit `Cookie.Domain` configured, so the auth cookie a successful login sets is scoped to whichever host served that POST. Signing in on `raniahmotamed.247advisers.com` (exactly what item 35 now sends visitors to) sets a cookie scoped to that domain, then redirects to `/Dashboard` on the *same* domain — and every other portal route except `/Account/*` is still swallowed by the public-website domain-rewrite, so the entire portal 404'd as "website not published" on every click after login. The user hit this directly and reported it with a screenshot. **Real fix**: `AccountController.Login` (GET) now checks `PortalUrlHelper.IsPlatformHost` (new helper, deliberately kept separate from `ShouldRouteToPublicWebsite` rather than refactoring that central routing function under time pressure) and, if the request arrived on an agent's own domain, redirects to the same login page on the platform's own base URL *before rendering the form* — so the actual credential POST, and the cookie it sets, happens on the right host. The footer/not-published-page links from item 35 are unchanged and still correct: they only need to get a visitor to a working login page, which now transparently hops to the platform domain if needed. Anyone with an already-broken session from before this fix needs to sign out and back in (or navigate directly to `https://ipro-prod-web.azurewebsites.net/Account/Login`) to get a correctly-scoped cookie - this fix prevents new occurrences, it doesn't repair sessions already in the broken state.
