...
- **Automatic overdue-invoice reminders**: once a sent invoice's due date passes, IPRO emails the client on the adviser's own schedule (523, 2026-09-26: before the due date, on it, the day after, then 7, 14 and 30 days overdue, each a switch with its own wording; `OverdueInvoiceReminderJob` at 9:00 a.m. Eastern, one stage per invoice, never two within five days) — nothing to set up, the defaults remind after the due date only. See `DOCS/10_CLIENT_INVOICING.md` ("Automatic Overdue Reminders").
...
- Agreed approach: redesign one flagship template first (**Editorial**, since it already had the strongest bones — serif type, real whitespace, shadow-free bordered cards), get sign-off, then roll the same visual language to Modern and Classic and eventually the newsletter wrapper. Modern and Classic got their own pass later — see item 58.
...
### 58. Roll Editorial's hero-fallback + section-rhythm redesign to Modern and Classic (done, 2026-07-29)

Item 31 fixed Editorial's weak Hero "no image" fallback and lack of section variety, deliberately scoped to Editorial only pending sign-off before rolling it further. Approved this session to bring the same fix to Modern and Classic — each adapted to that template's own established visual identity (Modern: dark navy-to-accent gradient hero, bold sans; Classic: light theme-tinted hero, Georgia serif), not a literal copy of Editorial's serif/dark-gradient CSS.

- **Modern**: replaced the generic Font Awesome compass icon inside the existing (already well-designed) `.mp-hero-media` box with an oversized, low-opacity monogram in Modern's own bold sans. Retokened `.mp-alt` (section rhythm) from a hardcoded gray to `var(--site-theme-soft)` — Modern already had broad rhythm coverage (9 block types, not just Editorial's 3), so only the hardcoded-color cleanup was needed there, not a behavior change.
- **Classic**: had zero Hero fallback at all (blank space, plus a genuinely empty reserved grid column for the `split`/`image-left` layouts specifically — the other 3 of 5 layout options never had this gap) and zero section rhythm anywhere. Added a monogram fallback (Georgia serif matching Classic's own `h1`; gradient background reusing the exact recipe already used for Classic's own CTA band, just a different angle) for the 2 layouts that actually need it, and a new `.cp-alt` tint (`var(--site-theme-soft)`) applied to Services, Reviews (non-banner variant), and TestimonialForm — the same 3 block types Editorial uses.
- `HeroMonogram` (agent's first-initial, falling back to company initial, falling back to "•") duplicated into both files rather than shared — matches the established fully-self-contained-per-template convention (no shared partial exists for this kind of logic in any of the 3 templates).
...
### 60. Add Video and Photo Gallery website blocks (done, 2026-07-29)

Two new block types, following the same `SettingsJson`-per-block pattern every other block uses (see `WebsiteReviewSettings`/`WebsiteMapSettings` for the established shape).

- **Video**: paste any YouTube link (watch/share/embed/shorts URL); `WebsiteVideoSettings.BuildEmbedUrl()` extracts the video ID via regex and builds a `youtube-nocookie.com` embed URL (YouTube's own privacy-enhanced embed domain — same player, no tracking cookie until the visitor presses play). Renders nothing on the live site if the pasted link isn't recognized, rather than showing a broken embed. Required extending the CSP's `frame-src` (`SecurityHeadersMiddleware.cs`) to allow the new iframe domain — Google Maps was previously the only one allowed there.
- **Photo Gallery**: multiple images per block, grid or carousel layout. This is the app's first true multi-file upload flow — no existing pattern to copy, so `UploadGalleryImage`/`DeleteGalleryImage` were built new in `WebsitePagesController`, reusing the same extension/content-type/magic-byte validation every other image upload in the app already uses. Deliberately **not** wired through the shared "Save Block" form/`ApplyBlockFieldsAsync` — a Gallery block's image list lives in its own `SettingsJson` and is mutated only by these two dedicated endpoints, so saving the block's Heading/Body never touches it.
  - **Storage quota decision (confirmed with the user)**: Gallery photos count against the *same* per-package storage limit already used for Agent Documents (`PackageFeatureCodes.FileUploadCapacity`; Silver 50MB / Gold 500MB / Platinum 1000MB / Broker 1000MB-per-user), rather than getting a new separate quota to configure. Since the image list lives inside `SettingsJson` (not a queryable byte column), usage is computed by loading the agent's own Gallery blocks (typically a handful) and summing in-memory — the same "SUM on read, no ledger table" approach `DocumentsController` already uses, just with an extra in-memory step. `WebsitePagesController.Edit` computes this once per page load (only if the page has a Gallery block) and the same storage bar UI from `Views/Documents/Index.cshtml` is reused on the block itself.
...
**Position — shipped 2026-07-26** (see item 44 below for full detail). Both axes of the original design are now live: 3 named themes and a real Sidebar Left/Right/None toggle usable with any of the 3 template families. Editorial's visual redesign rolled to Modern and Classic too — see item 58. Still open for a future pass: more themes toward the original 6-8.
