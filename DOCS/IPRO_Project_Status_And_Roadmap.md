...
**Status**: designed, not started — deliberately not started in the same session it was designed, given budget. This is the plan for the next session: build Position (the sidebar Left/Right/None toggle + graceful collapse) first since it's almost entirely reusing existing zones, then the Theme gallery (the real design effort — composing 6-8 genuinely good, distinct looks) second.
