...
| A2-H2 | Replacement can leave two live PayPal subscriptions | **Fixed** — reconciliation sweep in SubscriptionBillingJob converges duplicate Active rows (keeps newest, cancels the rest at PayPal first). Verified locally both ways: unreachable PayPal id → row stays Active + Error log naming both rows; clean cancel → stale row → Cancelled with CancelledAt, Warning log |
| A2-H3 | APPROVAL_PENDING/APPROVED accepted as "stopped" | **Fixed** — 422 accept-list shrunk to CANCELLED/EXPIRED; pending/approved now stay failures (visible, retried by the A2-H2 sweep). Runtime note: PayPal sandbox can't produce this 422 on demand; the narrowed branch is covered by the existing cancel-path sandbox tests |
| A2-H4 | Caps recorded, not enforced | **Split.** Trial half: **Fixed** — slot claimed via conditional UPDATE before account creation; claim released if registration throws. Verified locally including the real race (row lock held across the claim: pre-check passed at 4/5, rival took the last slot, claim returned 0 rows → clean rejection, no agent, no over-count). Promo half: the record-past-cap behavior is the **accepted interim by prior decision** — the reservation redesign is queued as pre-launch work before any real limited codes are issued |
...
| A2-H7 | Admin domain check misses equivalent hostname forms | **Fixed** — CanonicalizeDomain (trim/trailing dot/lowercase/IDN) + URL-component rejection + reserved platform zones + root/www variant collision check across AgentUsers, AgentWebsites and AgentDomains. Verified locally: root variant of another agent's binding rejected, `WWW.…CA.` canonicalized then rejected, `http://` rejected, reserved hostname rejected, own domain saves (stored lowercase) |
...
| A2-L1 | /health can be shadowed on agent custom domains | **Fixed** — ShouldRouteToPublicWebsite now consults IsNeverShadowedPrefix before the portal-controller check (the earlier never-shadowed addition was ineffective there; corrected attribution). Verified locally: `Host: bobmot1.247advisers.com` `/health` → `Healthy` |
