...
All items below: **fixed in commits `344ab9d`–`619a0e2` (2026-07-25).** Built, deployed, verified via GH Actions + real Azure container logs on both apps.

- ~~`DashboardController.cs`'s cached-insight staleness check queries by ID with no `AgentUserId` filter, inconsistent with the fallback branch three lines below it. Not currently exploitable (only affects whether the *current* agent's own cached text is kept or cleared).~~ **Fixed in `344ab9d`** — added the matching owner filter to all three branches.
- ~~`BillingIssueViewComponent.cs` has the same "NameIdentifier as AgentId, no auth-scheme check" shape as the original M-1 bug, currently safe only because it's exclusively invoked from the agent layout. No test/guard prevents a future change from reintroducing M-1 here.~~ **Fixed in `344ab9d`** — added the same `AuthenticationType` check Program.cs's gate middleware uses.
- ~~`EnsureUniqueIndexAsync`'s bare `catch {}` (added for L-12) will also silently swallow any *unrelated* failure (typo'd SQL, missing privilege), not just the documented duplicate-data case.~~ **Fixed in `b8d15ad`** — narrowed to `catch (MySqlException ex) when (ex.ErrorCode == MySqlErrorCode.DuplicateKeyEntry)` in both apps; anything else now propagates and crashes startup loudly, as intended.
- ~~`Views/Website/Index.cshtml.bak` (a stray, non-routable backup file) still has un-nonced inline `<script>` blocks — should have been caught by the L-10/11/13 dead-file cleanup.~~ **Deleted in `344ab9d`.**
- ~~160 tracked files under `src/IPRO.Web/publish/` — a committed build-output folder despite `.gitignore` excluding it...~~ **Untracked in `344ab9d`** (`git rm --cached -r`; `.gitignore` already excluded it, files remain on disk locally).
- ~~A stray root-level `output` file...~~ **Removed in `344ab9d`.**
- ~~The "read a base-URL config key, detect a placeholder, fall back to a hardcoded azurewebsites.net URL" pattern now exists in three independent copies...~~ **Fixed in `9dc7d3a`** — consolidated into `IPRO.Utility.WebAppUrlHelper.GetWebAppBaseUrl`, which checks both config keys (`App:BaseUrl` then `App:PortalBaseUrl`) so whichever is actually set in Azure keeps working; also folded in a 4th copy found in `OverdueInvoiceReminderJob.BuildInvoiceUrl` that wasn't in the original list.
- ~~L-2's fire-and-forget email fix has a small residual timing gap...and does its `GetRequiredService` calls outside its own try/catch...~~ **Fixed in `61589e0`** — added a flat 300ms delay on every `ForgotPassword` request (found or not), and moved scope/service resolution inside the task's own try/catch so a resolution failure can't become an unobserved task exception.
- ~~H-7's downgrade-to-billing-gate handoff isn't literally "every request" as described...~~ **Corrected in `619a0e2`** — the audit doc now describes the actual handoff (`ApplyDuePendingChangesAsync`, triggered by the hourly job or a Billing/Dashboard hit) instead of implying it's instantaneous.
- ~~The Billing page doesn't communicate "you were just downgraded to X, finish it here" after H-7's flow...~~ **Fixed in `619a0e2`** — the Billing page now shows a distinct "your downgrade to X is ready to finish" banner instead of the generic lapsed-billing one, whenever the agent is gated with a recently-applied downgrade on record.
