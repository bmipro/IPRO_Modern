...
| ~~416~~ | ~~Concept 02 (navy) is the chosen home-page direction — port it (#412)~~ | **PORTED + DEPLOYED 2026-08-18.** `Views/Home/Index.cshtml` is now the navy/aqua page. The prototype CSS and static markup were copied verbatim (assembly script, not retyped) so the design landed as designed; everything carrying live data is Razor. **Live, and must stay live if this page is edited again:** package columns render from `Model` (`BillingRule`) — monthly, annual, and the setup fee through `IsSetupFeeWaivedOn`, the same call that decides the PayPal charge; comparison rows render from `package.Features`/`LimitLabel`, not a hand-written list; `?package=` is carried into `/Account/Register` and `/Preview`; the hero portal mock reads `ViewBag.HeroInsight` (shared with `/Preview`); the demo address reads `ViewBag.TemporaryRootDomain` (#408 stays fixed); the SEO head from `6e04a5f` survived. **Deliberate deviations from the prototype, each for a reason:** (1) its contact form was `onsubmit="return false"` — inert by design, since a disk prototype has nowhere to post, and there is no contact endpoint on `HomeController`; shipping it would have put a form on the front door that silently ate every enquiry, so it became real CTAs (`/Preview`, `/Account/Register`, `support@iproadvisers.com`) and the section intro lost its now-false reference to "this form". (2) The base64 logo became `/images/ipro-advisers-logo.png`. (3) The inline script carries `nonce="@Context.GetCspNonce()"` or CSP would silently kill the market picker and the portal tabs. (4) The prototype's own RAZOR PORT NOTE was deleted — it was an instruction to the porter, and as an HTML comment it was shipping the internal $200/$400 fee amounts to page source. (5) Its header note that `/terms` and `/privacy` 404 is stale; both exist and return 200. Verified locally against real MySQL before deploy: $40/$60/$90 and the two waivers came from `BillingRules`, all five linked routes returned 200, no console errors, and at 375px the page body does not scroll sideways (the comparison table scrolls inside its own `overflow-x:auto` wrapper). The previous blue Bootstrap page is kept at `DOCS/marketing-site-prototype/homepage-blue-2026-08-16.cshtml`. HISTORY: Owner supplied "IPRO Concept 02" (outside AI concept, X:\ipro_related\web_template\main_site); massaged version committed 2026-08-13 (`49d56b3`) at `DOCS/marketing-site-prototype/concept02-navy.html`: Navy+Aqua committed (light `#173b55`/`#087d83`, dark navy fixed too), corp logo embedded, verticals = Insurance / Financial + Accountants + Mortgage + **Generic** (placeholder name, owner to decide), pricing corrected (annual 10×, Gold/Plat setup struck to $0 Waived, seats 1/2/5), 3 unverifiable claims cut (no per-vertical workflows; self-serve page builder instead of "review with a real person"; "guided human setup" trust item replaced), hero portal mock now interactive — 7 clickable tabs with keyboard support. Known limitation: <420px hides the mock's sidebar (pre-existing). **NOT deployed — it is a disk prototype.** Next: owner reviews + massage list, then the Razor port (data-driven pricing, CSP nonce scripts, real /Preview + ?package= links). Owner 2026-08-13: "too much for one day" — resumes next session. |
...
| ~~418~~ | ~~LATENT BUG — invoice numbers are reused after a full shred~~ **DEPLOYED 2026-09-09 in `455ee57`, verified at /health/version on BOTH hosts (build 6bd50f7).** | `IPRO-2026-000008` was issued TWICE: bob3test3 got 000008–000012, the shred deleted them, and the number generator (max+1 over surviving rows) handed 000008 to Bob2Mot the next day. Harmless while shreds are rare and test-only, but two customers sharing an invoice number is an accounting smell — the restore above had to key on the PayPal transaction id because the number is ambiguous. Fix direction: persist a high-water mark (or never decrement: number from a counter table, not MAX(Invoices)). **DONE:** NumberSequences (one row per key, one atomic UPDATE read back in the same transaction, seeded from the existing maximum on first use, safe inside a caller's transaction); both generators use it -- platform per year, client invoices per agent and document type. Five tests red-first, including 20 concurrent callers. |
...
| ~~419~~ | ~~Extract the ~30 duplicated schema-repair functions from BOTH Program.cs into one shared class~~ | **DONE 2026-08-18** (branch `fix/audit-high-five`, the A2-H8 half-close). 32 functions -> `IPRO.DataAccess.StartupSchemaRepair`; the extraction diff proved the copies had already drifted in 9 of 32 (comments/ordering only, never SQL -- luck, not design). Web Program.cs 2,019 -> 718 lines, Admin 1,776 -> 519. Verified: both apps boot the shared repair concurrently on the dev DB, and F3 re-proven -- an empty database boots to 98 tables with zero errors. EF-vs-repair unification stays open as its own initiative. |
...
| ~~410~~ | ~~MARKETING BUG — SMS on every plan~~ **VERIFIED ALREADY FIXED 2026-08-20**: the seeder row is no/no/no/no with "(not yet available)" AND RepairSmsReminderEntitlementAsync corrects previously-seeded databases at startup. Stale entry. | `PackageEntitlementSeeder.cs:185` seeds `SmsReminder` as included on all 4 packages; the data-driven "Compare all features" table renders a green check for a feature that DOESN'T EXIST. Copy fix on the page is not enough — the seeder row + live PackageFeatures rows must change (or the feature marked not-included) before any pricing page ships. Blocks the new pricing page. |
| ~~411~~ | ~~MARKETING BUG — pricing cards drop the chosen plan~~ **FIXED (home rebuild): every 'Get started' link carries ?package=; verified in Home/Index.cshtml on 2026-09-09 while inventorying open work.** | `Home/Index.cshtml` "Get Started" links go to `/Account/Register` with no `?package=`, though Register GET accepts it and preselects. Visitor picks Platinum, lands on an empty dropdown. One-line fix, real conversion cost. |
...
## OPEN -- one unidentified flaky test (observed 2026-08-28)

A full-suite run failed 1 of 396 on a tree that passed 396/396 immediately after, unchanged --
a flake, not a product defect. Its NAME WAS LOST because the failing run's output was piped
through `tail -3`. Standing instruction: full-suite gates must capture complete output (pipe to a
file, tail the file), and the next time a gate fails, grep the [FAIL] line before anything else.
Prime suspects: the timing-sensitive drip tests riding the UTC_TIMESTAMP() one-second boundary
(`Task.Delay(1100)` in DripRecoveryTests). Hunt it when it reappears with a name.
...
is a live promise to customers. Full detail, method and caveats: `DOCS/PRODUCT_TRUTH_2026-08-28.md`.
...
- **Possible aliases, confirm first:** `RotatingBanner` (the Hero block?), `CustomHomeButtons`
  (the CallToAction block?), `Newsboard` (the DidYouKnow mailer?).

**Method caveat:** absence of a name in the source is NOT proof a feature is missing -- it can
ship under another name. That produced one false alarm already: "Need analysis calculator" IS
real, as the `Calculator` block. Treat the list as questions, not findings.

**The honest pattern already in the data:** `SmsReminder` is labelled "(not yet available)" and is
included in ZERO packages. Anything not shipping should look like that.
...
| # | Item | Notes |
|---|---|---|
| ~~439~~ | ~~LAUNCH RISK: Gmail silently drops ACS mail (reproduced twice, 2026-08-31)~~ **RESOLVED 2026-08-31 -- there was no Gmail problem; one test mailbox. Ticked 2026-09-01 (the row already said so; the number was never struck).** | Two e-cards to bmot1966@gmail.com: ACS reported **Delivered** (3:09pm and 3:34pm, i.e. Gmail's servers returned 250) and the message is in NO Gmail folder -- `in:anywhere IPRO` covers Spam and Trash and returns 44 results, all Aug 5-12, none from Aug 31. Search is valid: the From header reads `IPRO Advisers <support@iproadvisers.com>` so the display name matches. **Gmail accepted then silently discarded** -- no bounce, no spam folder, nothing to mark 'not spam'. The hardest failure mode to detect and the reason to find it now. The SAME card reached bmotamed@gbssurveillance.com fine, so this is Gmail-specific, not a broken send. Likely cause: brand-new Azure sending infrastructure with zero reputation for this domain (every one of the 44 that DID arrive went via SendGrid's warmed pool), aggravated by e-cards being image-heavy with ~10 words of text -- a spam signal already flagged 2026-08-08. Headers cannot be read because the message never arrived. **OWNER DIAGNOSTICS:** (1) mail-tester.com -- send one e-card to its throwaway address, get an SPF/DKIM/DMARC + content score out of 10; this separates reputation from misconfiguration in 5 minutes. (2) Google Postmaster Tools (postmaster.google.com) -- add iproadvisers.com, verify by TXT; the only window into Gmail's view of the domain, worth having before launch regardless. **IF IT IS REPUTATION:** warm up by volume rather than blasting a first newsletter, and expect it to improve over days. **DO NOT** dismiss as 'it works for me' -- it worked to a non-Gmail mailbox both times. Gmail is a large share of any Canadian adviser's client list. **ROOT CAUSE FOUND 2026-08-31 via mail-tester (7.7/10, test-8uzjllxer): the entire content deduction is ONE SpamAssassin rule, FREEMAIL_FORGED_REPLYTO (+2.503; every other rule is +/-0.001 noise).** Authentication is clean -- SPF pass on 40.93.18.60, DKIM valid, DMARC passed; the only other flag is an unfixable rDNS mismatch on Microsoft's shared outbound host YT6PR01CU002.outbound.protection.outlook.com. So this was never a configuration problem. The message ships From `support@iproadvisers.com` (our authenticated domain) with Reply-To set to the sending agent's own email, and the owner sends as bahmanmotamed whose profile email is free webmail -- business-domain From plus freemail Reply-To is the header signature of business-email-compromise, and Gmail weighs it far harder than SpamAssassin does. Structural cause split out as item 440. NOT YET PROVEN that this alone is what Gmail drops on; the domain is also ~1 day old with zero history (mail-tester rates the IP only 'average', RCVD_IN_MSPIKE_H2). **DECIDING TEST, costs nothing:** send the same e-card to the same Gmail address from the Michael Tran demo agent -- michaeltran@alladvisers.com is not freemail, so same IP, same domain, same template, same reputation, and the Reply-To is the ONLY variable that changes. Lands => content, and 440 closes it. Still dropped => pure reputation, and the answer is warm-up + Postmaster Tools + time. **CONFIRMED 2026-08-31 evening -- YAHOO DELIVERS, GMAIL DISCARDS; it is the RECEIVING PROVIDER, not the message.** Every message-side variable is now eliminated by controlled test: freemail Reply-To (re-sent from the Michael Tran demo agent, business domain, 10/10 on mail-tester -- still dropped), image-heavy content (re-sent with natural greeting text -- still dropped), and the card format itself (sent a text-based E-LETTER -- still dropped). All three reported Delivered by ACS at 4:36 / 4:52 / 4:55pm, i.e. Gmail returned 250 and discarded. Gmail search is NOT at fault: `in:anywhere newer_than:2d` returns 15 messages from other senders and nothing from us, and `in:anywhere from:support@iproadvisers.com` returns 39 with the most recent Aug 12 -- the last SendGrid-era send. Meanwhile the SAME app, SAME code path and SAME ACS resource reached the inbox at Yahoo: raw headers on the 3:46pm unsubscribe notice show `atlas119.free.mail.ne1.yahoo.com`, `To: Bahman Motamed <bmotamed@yahoo.com>`, sent via YT5PR01CU002.outbound.protection.outlook.com / 40.93.18.48 / X-OriginatorOrg acsemailca1.onmicrosoft.com, with spf=pass dkim=pass dmarc=pass and Microsoft's own filter scoring SCL:1 (not spam) BCL:0 (not bulk). **THE 'IT WORKS FOR ME' TRAP IS LIVE AND THIS IS THE PROOF:** the owner's own agent notifications go to a YAHOO mailbox, so from inside the product email looks healthy while every client on Gmail receives nothing. **TWO CANDIDATE CAUSES, and Google Postmaster Tools separates them because it reports domain and IP reputation independently:** (a) DOMAIN reputation damaged by weeks of SendGrid sends to nonexistent QA mailboxes -- the same behaviour that got Twilio to suspend the account (item 431); reputation follows the DOMAIN, not the ESP, so the ACS migration reset nothing. (b) SUDDEN INFRASTRUCTURE CHANGE -- the domain jumped from SendGrid's warmed pool to Microsoft's SHARED ACS outbound range (40.93.18.0/24) with no warm-up ramp, which Gmail penalises on its own. **NEXT ACTION IS MEASUREMENT, NOT A FIX:** register iproadvisers.com at postmaster.google.com, verify by TXT, wait ~48h for data. Do NOT start remediation (dedicated marketing subdomain, warm-up ramp) before that report says which of (a)/(b) it is -- and note ACS's dedicated-IP support needs checking before any plan depends on it. Gmail is a large share of any Canadian adviser's client list, so this is the top launch risk at T-21 days. **RESOLVED 2026-08-31 evening -- THERE IS NO GMAIL PROBLEM. The prior conclusion on this row was WRONG and is retained above only so the reasoning error is visible.** A single e-letter sent to THREE recipients at once -- the control the whole investigation had been missing -- settles it: bahmanmotamed@gmail.com received it IN THE GMAIL INBOX in 4 seconds, test@iproadvisers.com received it, and only bmot1966@gmail.com did not. Same send, same second, same message. Gmail's own Show-original on the delivered copy reports SPF pass (2a01:111:f403:c103::1), DKIM pass (selector1-azurecomm-prod-net), DMARC pass, ARC pass, Microsoft SCL:1 BCL:0, multipart/alternative with a real text part, and List-Unsubscribe-Post: List-Unsubscribe=One-Click present (Google's bulk-sender requirement -- previously unverified, now confirmed). **ACS delivery to Gmail is healthy and this is NOT a launch blocker.** THE METHOD ERROR WORTH KEEPING: every 'Gmail drops our mail' conclusion from this session rested on ONE recipient mailbox, and a single-mailbox sample cannot distinguish a sender-reputation problem from one poisoned inbox. The owner proposed testing a second Gmail address; that suggestion is what broke it. Send to at least two independent mailboxes at the same provider before ever concluding a provider is filtering. WHAT REMAINS is a one-mailbox question, not a platform one: why bmot1966@gmail.com shows nothing. Leading explanation -- every Gmail screenshot today was mail.google.com/mail/u/0, so if u/0 is bahmanmotamed@gmail.com then mail was being sent to one address and searched in another all day, which also explains why the Aug 5-12 messages ARE present (they went to the searched address). Alternatives: a filter or block rule on that account after months of QA, or per-user spam learning. Google Postmaster Tools is still worth registering as pre-launch hygiene -- it is the only window into Gmail's view of the domain -- but it is NO LONGER URGENT and no remediation (marketing subdomain, warm-up ramp) is warranted. |
| ~~440~~ | ~~Reply-To is the sending agent's own email on every client-facing channel -- +2.5 spam points for any adviser on Gmail/Yahoo/Hotmail~~ **FIXED AND DEPLOYED 2026-09-01 in `20b9cd0` -- red-first, full gate 538/538, verified at /health/version on BOTH hosts.** | Split out of 439 and confirmed by mail-tester. All six client-facing senders pass `replyToEmail: agent.Email`: ECardDispatcher.cs:143, ELetterDispatcher.cs:115, NewsLetterDispatcher.cs:192/406/411, DidYouKnowEmailDispatchJob.cs:152. From stays on our authenticated domain, so whenever the adviser's own address is free webmail the message reads as business-From + freemail-Reply-To -- exactly what advance-fee and BEC fraud looks like, and SpamAssassin charges 2.503 for it. Nothing in the codebase knows what a freemail domain is (grepped). This is NOT a test artifact and NOT specific to the owner's account: a large share of independent Canadian advisers will sign up with a Gmail address, and each of them would carry this penalty on every card, letter, newsletter and drip step they ever send. Worth fixing even if 439 turns out to be reputation-driven, because 2.5 points is half the budget on any corporate mail server running SpamAssassin at the default threshold of 5. **RECOMMENDED FIX (awaiting owner decision):** set Reply-To to the adviser only when it is not freemail, and carry the adviser in the From display name instead (`Name via IPRO Advisers <support@iproadvisers.com>`) so the recipient still sees who it is from. Cost is small because the signature ALREADY renders the agent's email as a mailto: link in both the HTML and the plain-text alternative (ECardHtmlComposer.cs:137, :42) -- a client on a freemail adviser loses Reply routing but is one tap from emailing them directly. **PROPER FIX, POST-LAUNCH:** per-agent reply alias on our own domain (agent-1234@reply.iproadvisers.com forwarding to their real inbox) -- both headers on our domain, rule never fires for anyone, Reply keeps working for everyone. Needs INBOUND mail, which ACS does not do at all, so it is a new production dependency and days of work. Not three weeks from launch. **CONFIRMED 2026-08-31 from raw headers:** the owner's agent address is `bmotamed@yahoo.com` -- Yahoo, freemail -- so the Reply-To that fired FREEMAIL_FORGED_REPLYTO is identified, not inferred. Note this is INDEPENDENT of 439: fixing this does NOT fix the Gmail drops (proved by re-sending from a business-domain agent and still being discarded). Fix it for the 2.5 SpamAssassin points and for every future adviser on Gmail/Yahoo, not as a deliverability cure. **Superseded 2026-09-12 by 480:** the substitution sent every client reply of a webmail adviser to support; Reply-To is the adviser's own address again, on the owner's decision. |
| ~~441~~ | ~~A card that fails before sending shows Failed / Queued / blank Issue / 0 failed -- four contradictory signals and no reason anywhere in the portal~~ **FIXED AND DEPLOYED 2026-09-01 in `20b9cd0` -- red-first, full gate 538/538, verified at /health/version on BOTH hosts.** | Found while diagnosing the 4:29pm send on 2026-08-31. ECardDispatcher.FailAndReleaseAsync (src/IPRO.Email/ECardDispatcher.cs:214) writes the reason ONLY to the application log: it stamps ECards.Status=Failed and clears the claim, but never touches the recipient rows and persists the reason nowhere the UI can read. Result on Email Activity: card **Failed**, recipient still **Queued**, FailureReason empty so the Issue column is blank, and TotalSent 0 so the Failed COUNT reads 0. The agent is told the send failed and given nothing to act on; the only copy of the reason is the App Service log stream. Two reachable causes, both invisible: line 52 the sending agent record no longer exists, line 63 the card references an unknown design key. NOTE the two paths that already do this correctly for comparison -- the per-recipient consent check and the per-recipient catch both set recipient.Status=Failed WITH a FailureReason. FIX: persist the reason on the card (or fan it out to the queued recipient rows) so Email Activity can surface it, and make the recipient status terminal rather than leaving it Queued. Check ELetter/Poll/Newsletter dispatchers for the same shape before fixing -- ECard is unlikely to be the only one. |
| **442** | **LAUNCH BLOCKER: ACS sending quota is 30/minute, 100/hour -- the product cannot mail at launch scale. Azure support request 2608310040012537 filed 2026-08-31.** | Discovered on the ipro-prod-email domain Overview while chasing why Opened/Clicked never populate; this row also closes the long-open 'ACS sending quota still unchecked'. 100/hour is a PER-RESOURCE cap, not per-agent: one adviser's 500-client newsletter takes 5 hours, advisers sending the same morning queue behind each other, and birthday e-cards batch onto the same dates across the whole customer base. Requested 500/minute and 10,000/hour. THE SAME DEFAULT-LIMITS GATE ALSO BLOCKS USER ENGAGEMENT TRACKING, which is why opens and clicks are empty: Microsoft's docs state engagement tracking cannot be enabled on custom domains with default sending limits. Proof it is not our code -- the delivered message carries NO tracking pixel and its hrefs are NOT rewritten through an ACS redirector, so ACS has nothing to observe and can never emit an engagement event. AzureEmailEventsController already handles Microsoft.Communication.EmailEngagementTrackingReceived and maps View->open / Click->click, so nothing needs writing on our side; it starts working when the gate lifts. **APPROVAL IS NOT AUTOMATIC** -- Microsoft weighs sender reputation, and states delivery FAILURE RATES MUST BE BELOW 2%. iproadvisers.com carries the SendGrid bounce history from item 431, so until this is granted: NO test sends to addresses that cannot receive. test@iproadvisers.com now exists and is the only permitted QA target. Evaluation takes up to 72 hours. Owner has Azure Support Plan Standard, so the ticket carries no additional fee. WHEN GRANTED: verify engagement tracking actually applied by sending one e-letter and checking Show original -- an href rewritten to a Microsoft redirector means it is on; app.iproadvisers.com unchanged means it is not. NOTE the tradeoff before enabling: tracking rewrites every link through a Microsoft redirector and adds a pixel to every message. **TICKET CHANNEL, 2026-09-01:** Microsoft's first (automated) reply to the request bounced -- the ticket's only contact was bmotamed@iproadvisers.com, which sits behind the cPanel SpamAssassin on res-cp9.yyz2.websiteservername.com, and that filter REJECTED supportmail@techsupport.microsoft.com at SMTP time with 550 5.7.350 (31 Aug 22:42 UTC) -- the same filter that stamped our e-cards ***SPAM*** on 2026-08-08. Microsoft's own message was fully authenticated (it is sent via ACS too); this is the inbound filter, not reputation. Fix applied by owner: whitelisted *@techsupport.microsoft.com, *@microsoftsupport.com and *@microsoft.com (the three domains Microsoft's confirmation names) in cPanel Spam Filters; instructed to add bahman.motamed@gmail.com as a second contact on the request and to use the portal thread as the reliable channel. The assigned engineer (Vidhya Sagar Erra, ACS team, Sev B) asked for: expected daily/monthly volume after launch; volume sent on the domain in the last 30 days; and a screenshot of the domain Overview showing engagement tracking Off with no Turn On control. Reply drafted with the burst-not-aggregate justification (peak hour = everyone's newsletter morning), ~25 ACS messages since 30 Aug with zero bounces/complaints, and the hygiene controls; the only figures the owner supplies are the launch adviser/list counts and an honest estimate of the pre-30-Aug QA volume. Evaluation window is up to 72h from the reply. **2026-09-02:** the engineer came back for four remaining details -- company website, subscription ID, ACS resource name, maximum emails per day. Answered same day: iproadvisers.com; the production subscription; ipro-prod-comms + ipro-prod-email/iproadvisers.com (rg ipro-production); ~500/day typical, 10,000/day maximum at launch (consistent with the burst figures already on the ticket). Review continues from there. **2026-09-04 (read 09-08):** DECLINED on volume -- 28 messages in 30 days on the domain; 'send for one to two billing cycles or until throttling is observed, then re-apply'. **2026-09-08 reply sent (owner):** reframed -- the domain moved to ACS on 30 Aug so the window shows a migration, not the business; launch is 21 Sep; 'send until throttled' means throttling real customers on launch day; engagement tracking asked for first as a no-volume, no-abuse-surface change; the original 500/min, 10,000/hour restated; hygiene evidence (100% delivered, SPF/DKIM/DMARC, SCL:1, List-Unsubscribe one-click, per-recipient consent, bounce suppression, CASL). If declined again: plan for launch at 100/hour -- the dispatchers already queue and claim, so nothing breaks, but newsletters spread over hours; consider a second verified sending domain as a stopgap and re-apply with 30 days of launch data. **2026-09-10 (evening), Microsoft (Harsh Raj):** acknowledged the reframed reply, documented the details and shared them internally; request under review, no decision. **2026-09-11, reply sent (owner):** asked to handle the two parts separately -- enable engagement tracking on iproadvisers.com now (needed to finish testing before the 21 September launch, no change to the account's risk at today's volume), and give a date for the volume decision; if none before launch, we launch at current limits and re-apply after 30 days of data. |
| ~~443~~ | ~~Gmail ignores dots in addresses; our suppression does not -- a client entered twice under dot variants keeps receiving mail after unsubscribing~~ **FIXED AND DEPLOYED 2026-09-01 in `c3128d0` -- red-first, full gate 560/560, verified at /health/version on BOTH hosts.** | Found 2026-08-31 when a test to bahmanmotamed@gmail.com arrived as bahman.motamed@gmail.com. For gmail.com, Google strips periods from the local part, so john.smith@ and johnsmith@ are ONE mailbox. IsSuppressed keys on client.EmailOptOutAt, a per-CLIENT-ROW flag (EmailConsentService.cs:105), so two rows for the same human are independent: they receive every card, letter, newsletter and drip step TWICE, and unsubscribing one row leaves the other mailing them. The second half is the CASL exposure -- an unsubscribe has to actually stop the mail. Duplicate sends are also a direct route to spam complaints, which now feed the 442 quota review. FIX: normalise gmail.com addresses (strip dots, strip +tag) when checking for duplicates on client import AND when resolving suppression. Do NOT normalise other domains -- dot-stripping is a gmail.com-specific rule and applying it elsewhere would merge genuinely different people. Not a launch blocker; cheap insurance. |
| ~~444~~ | ~~Opened and Clicked show a bare em-dash that cannot distinguish 'nobody opened it' from 'tracking is not enabled'~~ **FIXED AND DEPLOYED 2026-09-01 in `f431f5b` -- red-first, full gate 567/567, verified at /health/version on BOTH hosts. Opens/clicks say 'not tracked' until Email__EngagementTrackingEnabled is flipped after 442 is granted (runbook has the steps).** | Same defect class as 441: a true state the UI cannot express, leaving the agent to guess. With 442 outstanding, engagement tracking is OFF at the provider, so every row in Email Activity shows blank opens and clicks and there is nothing telling the agent why. Once 442 is granted the columns start populating for NEW sends only, and historical rows stay blank forever -- which will look like a bug unless the UI says otherwise. FIX: surface provider tracking state so the columns read 'not tracked' rather than an ambiguous dash. Also worth keeping the existing footnote that opens depend on the recipient's mail client loading images, so the real rate is always higher than shown. |
| ~~445~~ | ~~LAUNCH RISK: a deploy can leave the web container STOPPED -- production returned 503 for ~24 minutes on 2026-09-01, and the only alarm was a GitHub failure email~~ **FIXED AND DEPLOYED 2026-09-01 -- all five fixes live and verified. (5) alerting, owner, portal: action group `ipro-prod-oncall` (email to Yahoo + SMS, both proven delivered), Standard availability tests `app-health` and `admin-health` on /health from 5 regions every 5 min, alert on 2-of-5 failures at Severity 1, wired to the group on both rules; first 30 minutes of probes 25/25 and 14/14 green, 200-1400 ms. A dead container now pages a human within ~10 minutes, traffic or not. Deploy-side: (1) f58c320, (2) a466ffe, (4) 1a7b29e; (3) container stdout capture on both apps.** | Timeline (local time): docs-only commit 8fb682c pushed 15:53; the webapps-deploy step ran and the app kept serving c3128d0 until 15:55:19, then `<no response>`; the workflow's own verify step correctly failed at 16:02 after its 5-minute poll; every path on BOTH the custom domain and ipro-prod-web.azurewebsites.net returned HTTP 503 in ~0.1s (front door up, worker dead); `az webapp show` reported state=Running / availability=Normal while `az webapp list-instances` showed the SINGLE worker instance STOPPED; the log stream was silent (nothing running to log); health-check path /health on a single-instance Basic SKU means one unhealthy instance = total outage. Remedy, with the owner's explicit go: `az webapp restart -n ipro-prod-web -g ipro-production` at 16:18:55 -> instance READY -> HTTP 200 serving 8fb682c at 16:19:21. admin was unaffected. ROOT CAUSE UNPROVEN: the build was docs-only (identical code to c3128d0, which came up cleanly 30 minutes earlier), and 20b9cd0 + c3128d0 both deployed cleanly the same day, so this looks like a platform-side restart race rather than anything in the package -- but that is inference, not evidence. WHAT IS WRONG STRUCTURALLY: the pipeline DETECTS this (the verify step exists precisely for it and did its job) but nothing REMEDIATES it and nobody is paged; the owner found out from a GitHub Actions email. On launch week an unattended 503 for hours is not acceptable. FIXES, in priority order: (1) in the deploy workflow, when the verify step fails, run ONE `az webapp restart` and re-verify before failing the job -- turns a 24-minute outage into ~1 minute and needs only the deploy credentials the workflow already holds; (2) an Azure Monitor availability test on https://app.iproadvisers.com/health with an action group that emails AND texts the owner, so a 503 pages a human instead of landing in a GitHub notification; (3) look at App Service > Diagnose and solve problems > 'Web app restarted' / container logs for the 15:55 window to turn the inference into a cause; (4) consider App Service auto-heal rules once (3) says what to trigger on. STANDING REMEDY until then: `az webapp list-instances` -- STOPPED vs READY is the tell -- then `az webapp restart`, then /health/version on BOTH hosts. **FIX (1) SHIPPED 2026-09-01 in `f58c320`:** both deploy workflows' verify step now gets a second chance -- if the first 5-minute poll fails it emits a ::warning, issues ONE more `az webapp restart` where the deploy credentials already are, and polls again; it still fails loudly if that does not take. Pinned by DeployWorkflowTests (observed RED on the old YAML). The deploy of f58c320 itself came up cleanly on both hosts, so the second chance has not yet fired in anger -- when it does, the run log shows the ::warning. NOT TICKED: this is remediation, not a cure. Still open, in order: (2) an Azure Monitor availability test on https://app.iproadvisers.com/health with an action group that emails AND texts the owner -- a 503 must page a human, not land in a GitHub email; (3) App Service > Diagnose and solve problems for the 15:55 window on 09-01 to turn 'platform restart race' from inference into a cause; (4) auto-heal rules once (3) says what to trigger on. **ROOT CAUSE, evidence-backed (2026-09-01 evening) -- the 'platform race' inference above is WITHDRAWN.** The web container log shows the app process ABORTED during startup with exit code 134 (SIGABRT = unhandled .NET exception on Linux) FIVE times in a row, 40-47s in, at 19:56, 19:57, 19:59, 20:01 and 20:08 UTC, with the platform backing off 1-2 min between attempts, then recovered ON ITS OWN on attempt 7 at 20:11 UTC (16:12 local) -- six minutes BEFORE the manual restart, which was therefore a coincidence, not the fix. Zero 134s on any of the previous seven days. Attempt 1 was not a crash: it was KILLED at 13s by our own workflow's explicit `az webapp restart`, which fires ~15s after the deploy's own restart has already begun -- every web deploy today shows this double-restart signature (probe 'failed' at 1.7s / 3.8s / 13s, then a clean second start). The production MySQL (ipro-mysql-prod, Burstable B1ms) was HEALTHY throughout: cpu 9-11%, memory flat 24.5%, aborted_connections 0, cpu_credits_remaining 312 all day -- the active_connections drop 21->8 at 19:57 is the web app's own pool being absent. MECHANISM (coherent with every datum, unproven only because the app's stderr was not captured): Program.cs runs ~20 un-guarded StartupSchemaRepair.Ensure*SchemaAsync calls -- live `ALTER TABLE ... ADD COLUMN` / CREATE TABLE IF NOT EXISTS DDL -- plus MigrateAsync and seeders on the main thread before app.Run(). The double restart killed attempt 1 mid-DDL; a killed process's MySQL session can linger holding a METADATA LOCK; every later startup's DDL then blocked on it until MySqlConnector's default 30s command timeout threw (10s of startup + 30s = the 40-47s crash point), unhandled -> 134, until the zombie session was reaped (~15 min) and attempt 7 sailed through. A lock wait burns no CPU, hence the pristine DB metrics. THE DIFFERENTIAL THAT CLINCHES IT: ipro-prod-admin runs the SAME startup block against the SAME database and has NEVER crashed -- because it lives in resource group `ipro-prod-admin_group`, not `ipro-production`, so the workflow's 'best effort' admin restart has FAILED SILENTLY on every deploy (misreported as 'no permission') and admin only ever restarts ONCE. Web restarts twice; web crashes. FIXES, revised: (1) SHIPPED f58c320 -- second-chance restart; still worth having but it treats the symptom. (2) STOP THE DOUBLE RESTART: make the explicit restart conditional -- poll /health/version ~90s after the deploy step and restart ONLY if the old SHA is still serving (keeps the WEBSITE_RUN_FROM_PACKAGE safeguard the step exists for, never kills an in-progress start); apply the same shape to admin with its real RG rather than 'fixing' the RG and giving admin the same disease. (3) CAPTURE STDERR: `az webapp log config --docker-container-logging filesystem` on both apps (NOT --application-logging, which auto-disables after 12h) so the next 134 leaves a stack trace in *_default_docker.log. (4) HARDEN STARTUP: the Ensure*Schema DDL should not run un-guarded on every production start -- at minimum wrap like the seeders (log and continue), set an explicit command timeout, and decide whether repair DDL belongs in migrations instead; a startup that cannot get a lock must not take the site down. (5) The paging availability alert stands. Still open; not ticked. **STATUS AT 2026-09-01 CLOSE -- three of five fixes shipped and verified on both hosts; NOT ticked because (5) is open.** (2) SHIPPED `a466ffe`: the explicit post-deploy restart is conditional -- polls /health/version for 90s and fires only if the OLD build is still serving; admin addressed in its real resource group `ipro-prod-admin_group`, which also repaired the second-chance restart from f58c320. First live run: web saw the old build through checks 5-7 while the deploy's own restart finished, then 'no restart needed' -- the double restart is gone. (3) DONE: `--docker-container-logging filesystem` on both apps; the next exit-134 leaves a stack trace in *_default_docker.log. (4) SHIPPED `1a7b29e`: StartupGuard in IPRO.DataAccess, applied to both Program.cs files -- every repair, guard, seeder and blob call wrapped (log + stderr + tracker clear, continue; 36 steps in Web, 35 in Admin), a MySQL advisory lock held for the block on a connection kept open for the scope so web and admin serialise (timeout proceeds unlocked with a warning, never skips, never waits forever), and a 15s command + lock_wait timeout armed AFTER MigrateAsync so a real migration is never cut off. MigrateAsync is the one step left unwrapped, on purpose. Red 5/6 -> green 6/6 including a real-MySQL lock test via IS_USED_LOCK from a second connection. REMAINING: (5) the paging alert -- owner, portal: Azure Monitor availability test on /health with an action group that emails AND texts. Until then a 503 is still only a GitHub email. **FIRST STARTUPGUARD BOOT, 2026-09-01 22:03 UTC (1a7b29e):** platform log shows one container start, startup probe succeeded after 35.0s, Site started -- no retry, no 134. The app-stdout capture (fix 3) is confirmed working: *_default_docker.log now carries the container bootstrap for every boot since it was switched on. The app's own Information-level lines -- including StartupGuard's 'holding advisory lock' -- are NOT emitted in production (console logger is Warning+), so a silent app section means a clean boot; a failed step logs at Error AND to stderr and WILL appear. SMALL FOLLOW-UP: echo the lock outcome to stdout unconditionally so the log positively confirms serialisation rather than implying it by silence. |
| ~~446~~ | ~~Portal links built as strings 404 on an agent's own domain -- the AI Daily Assistant's 'View' was the first click of the owner's first morning of testing on his own host~~ **FIXED AND DEPLOYED 2026-09-02 in `595d4a1` -- red-first (14/21 on an identity skeleton), full gate 599/600 with the one failure a named flake (447), verified at /health/version on BOTH hosts. Insight rows stored before the fix are repaired on read; no migration.** | Found 2026-09-02 on bahmanmotamed.247advisers.com and reproduced on the connected domain www.4ipro.com: 'View' under Suggested next action went to /WebsiteLeads?status=new and got the public site's 404. THE URL-SPACE RULE (INVARIANTS; Program.cs ShouldRouteToPublicWebsite): on an agent host a bare path is ALWAYS the public website; the portal lives ONLY under /portal. Tag-helper links comply because the portal route is registered first; string-built links did not, in FIVE writers -- AiDailyDigestJob (three suggested-action URLs, PERSISTED on the insight), ClientsController client-timeline links (added/completed), MarketingCalendarController events (newsletter, social post, drip step), WebsiteLeadsController.SafeReturnUrl (supplied and default). All of them worked on app.iproadvisers.com, which is why nobody saw it. Account and Billing are never-shadowed prefixes by design and are correct on every host -- the billing gate's Redirect('/Billing') is NOT a defect. FIX: PortalPaths in IPRO.Utility (Web, Scheduler and Business all reference it) -- idempotent, absolute/protocol-relative untouched, blank -> null -- applied at every writer, plus read-time normalisation on the dashboard so insight rows already stored bare display correctly without a migration. GUARD: a regression sweep over Controllers, Scheduler and Business fails on any new bare string link into a shadowed portal controller; its first version caught one straggler the initial grep missed (the social-post calendar event). |
| ~~447~~ | ~~The flaky test, now identified~~ **MEASURES DEPLOYED 2026-09-02 in `bae8929` (build 3948647): parallelism capped at 4 -- 687/687 in 23m47s and 697/697 in 27m03s, zero failures across both; the overdue test names its cause. Reopen only if a load-only failure recurs under the cap.** | **IDENTIFIED 2026-09-02:** the flaky test is `SendClaimsTests.Retiring_a_poll_send_that_mailed_nobody_returns_the_survey_to_draft` -- failed once at 19s into a full parallel run, passed 3/3 alone. Almost certainly a shared-state race with a sibling test under parallel load; investigate the fixture, do not retry-loop it. **2026-09-02, second name:** `MediumAuditFixTests.Overdue_job_reminds_only_entitled_agents_and_stamps_the_marker` failed once in the 451 gate (616/617), green alone in 13 s; own database, own recording mailer, no Hangfire. Same shape as the first: load-only. Two of these now -- look for what the fixture shares under parallel load (per-test databases restart ids at 1; anything keyed by id across the process would collide) before touching either test. **MEASURES 2026-09-02:** no shared state found in either test; both sit on code that talks to the single local MySQL under a full parallel run, and the overdue job catches-and-logs per invoice, so a database hiccup surfaced as 'Assert.Single failed' with no cause. Two changes: (1) xunit.runner.json caps maxParallelThreads at 4 (measured on the next gates: duration and failures); (2) the overdue test injects a recording error logger and asserts the job logged nothing, so the next failure names its cause. Gate logs are now kept in full in the scratchpad. **2026-09-08:** two more gates under the cap, 700/700 in 31m58s and the 464(2)/465 gate (see log): zero failures; duration varies with what else the box is doing, the failures did not return. |
| ~~448~~ | ~~Drip campaigns: 'send immediately' waited for the hourly job, drip sends were invisible in Email Activity, and two job runs could both mail the same enrollment~~ **FIXED AND DEPLOYED 2026-09-02 in `92263ee` -- red-first (10/13 on a skeleton; the enqueue proven by mutation), full gate 614/614 then combined gate green, verified at /health/version on BOTH hosts. The 12:00 hourly run had already proved the old pipeline end to end (step 1 sent, enrolment advanced to step 2) before this deployed.** | Found 2026-09-02 by the owner on his first drip: enrolled a client in a 3-step campaign, saw 'send immediately', got nothing for an hour (DripCampaignJob is Cron.Hourly; newsletters go minutely, which is why the newsletter he sent at the same time arrived at once), then could not find the send anywhere -- Email Activity never listed DripCampaignStepSends although every send is recorded there, and the Campaign page's Performance panel showed zeros captioned 'SendGrid events'. While in there: the hourly job had NO guard against overlapping runs. FIX, one change: (1) enrolment enqueues a one-off DripCampaignJob.RunEnrollmentAsync (Hangfire 'drip' queue) for each enrollment whose first step is due; the job and the hourly run share DripEnrollmentClaims -- SendClaims' pattern on ClaimedAt/ClaimAttempts without a status transition, 15-min stale window, 3-attempt cap, exhausted claims named Failed rather than silently excluded, Resume clears the slate; two new columns via the schema repair. (2) Email Activity gains a Campaigns tab with per-step rows and recipient detail (EmailActivityQueries, tested on real MySQL). (3) Performance panel is provider-neutral and says 'nothing sent yet'. Red-first (10/13 on a skeleton); the enqueue is a behavioural test through the real controller, proven by mutation. |
| ~~449~~ | ~~Page editor Image Library rendered uploads as a 4-across gallery (16:10 cards, two stacked full-width buttons) -- 'almost distracting'~~ **FIXED AND DEPLOYED 2026-09-02 in `a0e8e4e` -- pin observed RED on the old markup via stash, GREEN after; combined full gate green; verified on BOTH hosts.** | Owner, 2026-09-02, on /portal/WebsitePages/Edit. It is a picker: now col-4/col-md-3/col-lg-2 (six across on a desktop), 4:3 thumbnails, Use + Remove side by side and small, Remove icon-only with an accessible name. JS hooks (uploaded-image-choice, data-image-*) and the delete form unchanged and pinned by ImageLibraryCompactTests, observed RED on the old markup. |
| ~~450~~ | ~~FEATURE, OWNER TO DECIDE: let website visitors vote in a poll on the page (today polls are email-only)~~ **DEPLOYED 2026-09-02 in `48d997a`, verified at /health/version on BOTH hosts (build 3948647).** | Raised 2026-09-02 while testing the PollResults block. What exists: a poll goes to named clients by email; each PollRecipient carries a VoteToken; PollVoteController records one vote per emailed client; the PollResults website block shows aggregate results only once TotalResponded >= 10 (PollResultsBuilder.PollResultsMinResponses) -- an ANONYMITY floor, because with 2-3 responses a visitor who knows the clients can work out who answered what. Keep the floor. What does not exist: any anonymous vote path. A visitor-vote block is a new feature, not a setting: a PollVote block type rendering question + options + vote; anonymous responses (ClientId null, Source=Website) with a repeat-vote cookie and light abuse limits; standard web-poll UX -- vote, then see aggregate results; the anonymity floor still governs results that include emailed clients. Three renderers, the block picker, a public POST endpoint, results merge, tests. Roughly a day. Not a launch blocker; recommended post-launch unless the owner wants it as a launch differentiator. **BUILT 2026-09-02 (pending deploy; owner: 'lets do the still open'):** PollVote block, anonymous Website recipients (Source column), cookie + hourly cap, results at once for never-emailed polls and the ten-response floor when clients were emailed, the agent's Results page counts website votes; guide section in 16_POLLS_AND_SURVEYS.md. Ten tests red-first. |
| ~~451~~ | ~~Container swaps kill MySQL sessions mid-work and their locks outlive them -- the root under 445's outage AND today's Hangfire stall~~ **DONE 2026-09-02: setting on both apps 13:49 (owner's go), code half in `73bb14d`, verified at /health/version on BOTH hosts (b3c3004).** | Seen 2026-09-02 16:37 UTC on the first boot after 448: six `Hangfire.Storage.MySql.JobQueue.MySqlJobQueue` 'Command Timeout expired / Connection timed out' lines from three workers at the same instant, one minute after a clean start, then nothing; DB metrics show 0 aborted connections and a 17->29 connection bump that minute. Yesterday's five boots had none. MECHANISM: App Service stops the old container with a 5-second grace period (docker log: 'Container is terminating. Grace period: 5 seconds'); Hangfire's server ShutdownTimeout is 15s by default; the old container is killed before Hangfire finishes, its dequeue transaction on Hangfire_JobQueue never rolls back, and MySQL keeps the dead session's row lock until it notices the peer is gone. The new container's workers block on that row, hit the 30s command timeout, log at fail level, retry, and recover once the lock is reaped. Same root as 445 (a killed session's METADATA lock during startup DDL), which StartupGuard now survives; this one is self-healing but noisy and it will keep happening on any deploy that catches a worker mid-dequeue. FIX, two halves, both cheap: (1) OWNER, App Service setting on BOTH apps: WEBSITES_CONTAINER_STOP_TIME_LIMIT = 30 (seconds), so a stopping container gets long enough for Hangfire and EF to shut down and roll back -- a settings change restarts the app, so do it once, at a quiet moment; (2) CODE: AddHangfireServer(o => o.ShutdownTimeout = TimeSpan.FromSeconds(10)) in both Program.cs files so Hangfire's own stop fits inside that window with margin. Not urgent, not a launch blocker; worth doing before launch week because it removes the whole class. **(1) DONE 2026-09-02 13:49 -- WEBSITES_CONTAINER_STOP_TIME_LIMIT=30 set on both apps via `az webapp config appsettings set` (output suppressed; verified by name), both apps answering 200 throughout. (2) code half: Hangfire ShutdownTimeout, next push.** |
| ~~452~~ | ~~Client invoices are not tracked after Send -- the agent cannot tell whether the client received one (owner, 2026-09-02: "I think we should")~~ **DEPLOYED 2026-09-02 in `eda4efa`, verified at /health/version on BOTH hosts (build 9ea67ac).** | Reviewed on the owner's question. ClientInvoicesController.Send stamps Status=Sent and SentAt BEFORE the email is attempted, calls SendDetailedAsync and DISCARDS the EmailSendResult, then shows 'Invoice sent to x' regardless -- a 429 from the 30/minute ACS quota (442), a bad address or a provider outage is invisible to the agent. No ProviderMessageId is stored on the invoice, so the Event Grid resolver (AzureEmailEventsController.ResolveByMessageIdAsync: newsletters, drips, e-cards, e-letters, polls) cannot match Delivered/Bounced to it and invoices do not appear in Email Activity. The public document page (ClientDocumentController.Show via ViewToken) records nothing when the client opens it, although that is the strongest 'received' signal and needs no provider. OverdueInvoiceReminderJob has the same blind spot. PROPOSED (about half a day, red-first): (1) a ClientInvoiceEmail row per send/resend/reminder with ProviderMessageId, Delivered/Bounced/at, failure reason; (2) check the send result -- on failure keep the invoice Sent but surface 'could not be delivered: reason' instead of a success banner; (3) add invoices to the resolver (a hard bounce already suppresses the client); (4) FirstViewedAt/ViewCount stamped by the public page, shown as 'Viewed <date>' on Details and Index; (5) optionally an Invoices tab in Email Activity. Recommended before launch: invoicing is a paid entitlement and a silent send failure is money-facing. Owner to decide. |
| ~~453~~ | ~~Support tickets: (1) a dedicated inbound address, (2) open tickets on the SuperAdmin dashboard, (3) the reply email must say do-not-reply / log in to continue (owner, 2026-09-02, testing ticket submission)~~ **DEPLOYED 2026-09-02 in `cd46a25`, verified at /health/version on BOTH hosts (build 9ea67ac).** | Owner's three points while testing support-ticket submission. (2) and (3) are straightforward and go in after 452: a dashboard panel in Admin listing open/awaiting-reply tickets with age and agent, and a footer on the reply notification: 'Please do not reply to this email. To continue, sign in at https://app.iproadvisers.com/Account/Login and open Support / My Tickets.' (1) needs a decision: ACS is send-only, so a ticket@ mailbox that CREATES tickets means polling a cPanel mailbox over IMAP and parsing replies -- a real subsystem, and a spam surface. Cheaper and safer: keep ticket creation in the portal and send the SuperAdmin a notification for every new ticket and every agent reply to support@iproadvisers.com, so nothing waits on someone opening the dashboard. Recommend that form of (1). **BUILT 2026-09-02 (pending deploy):** (2) the dashboard panel -- unanswered first, then longest-waiting, red count badge; (3) the footer + 'Open your ticket' button on the reply email (URLs from App:BaseUrl); (1) kept as the existing notification to Support:NotificationEmail, now pinned by a test. Four tests red-first (3/4, the pin passing), green after. |
| ~~454~~ | ~~Four client-facing one-off senders still discard the provider's answer and tell the agent 'sent' regardless -- the 452 blind spot, elsewhere~~ **DEPLOYED 2026-09-02 in `09c0c82`, verified at /health/version on BOTH hosts (build 9ea67ac).** | Sweep on 2026-09-02 after the owner asked whether 'Invited, pending activation' meant the invite email had gone out. It means the Invite button was clicked and a 7-day activation token exists; the email was attempted once at that moment, its EmailSendResult discarded, and 'Portal invite sent to x' shown either way. Same pattern in PortalRequestsController (appointment confirmed / declined emails, x2) and TestimonialsController (testimonial request). The other nine discarding call sites (trial reminders, PayPal action-needed, unsubscribe notifier, cert/domain ops mail, support notifications) mail the agent or ops and log on failure, which is acceptable. **2026-09-11 (the rest): the two ops senders that also ignored a returned false (certificate expiry, domain automation) now log a warning when the provider refuses; every other sender already checks. Nothing left under 454.** FIX (an hour): capture the result at the four client-facing sites; on failure show 'could not be sent to x: reason' instead of a success banner, and for the portal invite stamp PortalInviteSentAt / PortalInviteFailure on the client so the profile can say 'invite emailed Sep 2, 3:10 pm' or 'invite could not be sent'. The activation link already shown on the profile stays as the manual fallback. **BUILT 2026-09-02 (pending deploy, owner: 'lets do them all'):** all four sites capture the result; the portal invite remembers PortalInviteEmailedAt / PortalInviteEmailError (schema repair) and the card shows 'Invite emailed <when>' or the failure; appointment and testimonial failures replace the success banner with the reason. Six tests red-first. |
| ~~455~~ | ~~Client-portal Logout answers 400 Bad Request (owner, 2026-09-02, first click)~~ **DEPLOYED 2026-09-02 in `f320512`, verified at /health/version on BOTH hosts (build 9ea67ac).** | The logout form renders on pages authorized with the ClientPortal scheme, so its antiforgery token is bound to the CLIENT identity; ClientPortalAccountController is [AllowAnonymous] and Logout had no Authorize of its own, so on the POST the authorization middleware never installed the client principal -- HttpContext.User was the default scheme's (the agent cookie when the owner was also signed in as an agent, else anonymous) and the antiforgery filter refused the token as 'meant for a different user'. FIX: [Authorize(AuthenticationSchemes = "ClientPortal")] on Logout so the token is validated against the identity that generated it; pinned in source (no HTTP harness in the test project). |
| ~~456~~ | ~~Branded error pages for 400 / 403 / 404 / 405 / 500 instead of the browser's own (owner, 2026-09-02)~~ **DEPLOYED 2026-09-02 in `69651fd`, verified at /health/version on BOTH hosts (build 9ea67ac).** | Today only unhandled exceptions get a page (UseExceptionHandler -> /Home/Error, a standalone Bootstrap card); every other status returns an empty body, so Firefox shows its fox. PLAN: UseStatusCodePagesWithReExecute('/error/{code}') applied only to requests that accept HTML and are not machine endpoints (/health*, the ACS and SendGrid event webhooks, the PayPal webhook), an anonymous ErrorController that keeps the status code and picks the message per code, one standalone branded page whose 'back' link depends on where the visitor was (/portal, /ClientPortal, the site home), and the same for Admin. Phase 2: on an agent host, a 404 in the public site's own design. Tests: controller unit tests per code, helper tests for the HTML/machine predicate, wiring pins. After 455. **OWNER 2026-09-02: 'lets build it ... for now we go with server level errors and later we could change it to agent level as well.' Building platform-level pages now; the agent-level step is 458. Documented in DOCS/ERROR_PAGES.md. |
| ~~457~~ | ~~Client-portal links point at two different hosts (found 2026-09-02 answering 'what is the URL to the client portal?')~~ **DEPLOYED 2026-09-02 in `de4d8b5`, verified at /health/version on BOTH hosts (build 9ea67ac).** | The client-facing prefixes (clientportal, clientportalaccount, ...) are never-shadowed precisely so a client reaches the portal on the AGENT'S domain (https://<agent-domain>/ClientPortalAccount/Login). But ClientsController.InvitePortal builds the emailed activation link from App:BaseUrl, so every client lands on app.iproadvisers.com/ClientPortal -- the platform's host, not their adviser's -- while the profile page shows the same link as https://<agent-domain>/portal/ClientPortalAccount/Activate (works, but the /portal form is the agent-portal route, not the client one). Both work today; the inconsistency is branding and confusion. FIX (small): build the invite link from the agent's own host (the primary AgentDomain when connected, else <subdomain>.247advisers.com) with the bare /ClientPortalAccount path, and make the profile page show that same URL. Advise; not launch-blocking. **BUILT 2026-09-02 (pending deploy):** ClientPortalUrls resolver (healthy custom domain > free subdomain > platform); the invite email and the profile card use it; the card shows the sign-in address while invited AND once active (owner: 'somewhere that url need to be available'). Six tests red-first. |
| ~~458~~ | ~~FUTURE: agent-level error pages -- on an agent's own domain, a public-site 404 in the site's own design (header, colours, navigation), and optionally a per-website '404 page' block the agent can write~~ **ALREADY BUILT -- verified live 2026-09-02.** A missing page on a live agent site renders inside the site's own shell (header, navigation, footer, the agent's accent colour, a drawn 404, 'We couldn't find that page', a home button and page suggestions), status 404 with noindex -- `_PublicPageNotFound.cshtml`, in all three templates, set by `PublicWebsiteController` when the slug is not a page. Checked on bahmanmotamed.247advisers.com/no-such-page: 404 + the site shell. The platform-branded page (456) covers everything that is not a public-site page. Only the optional per-website custom 404 message from the recipe is unbuilt; log it if an agent asks. |
| ~~462~~ | ~~Small findings from writing the guides (2026-09-02), none launch-blocking~~ **ALL SIX CLOSED; (a)(d)(e) DEPLOYED 2026-09-09 in `9b89ad2`, verified at /health/version on BOTH hosts (build ed0cd56)** | (a) ~~The Did You Know block editor's help text still says a visitor 'sees the full content immediately'; the build emails the articles -- fix the sentence.~~ **DONE 2026-09-09: the sentence says the articles are emailed one at a time.** (b) ~~CalendarReminderJob (hourly) emails one hour before a CalendarEvents row, but no portal UI creates those rows -- dead job; remove it or wire follow-ups to it.~~ **DEPLOYED 2026-09-09 in `cea3ac8`, verified at /health/version on BOTH hosts (build 7cb630b): removed; the stale Hangfire definition is dropped at startup.** (c) ~~Marketing Calendar places events by calendar date without the agent time-zone conversion Email Activity uses -- near-midnight events land on the neighbouring day.~~ **DEPLOYED 2026-09-09 in `cea3ac8`, verified at /health/version on BOTH hosts (build 7cb630b): events on the agent's local date, month bounded by local midnights.** (d) ~~Articles list: Delete is permanent with no confirmation -- add the js-confirm-submit pattern.~~ **DONE 2026-09-09.** (e) ~~DOCS/04 says the Blog block is Platinum/Broker; no per-block check exists in code (the editor is gated on Instant Website) -- align the doc or the code.~~ **RESOLVED 2026-09-09: the premise was stale -- adding a Blog block has been gated on ManagedBlog at the POST since 2026-08-28, so doc and code agree; a test now pins them together.** (f) ~~PackageFeatureCodes.DidYouKnowManager is seeded for every package and checked nowhere~~ -- resolved by 464 (withdrawn). |
| 463 | PROSPECT: corporate customer (Masoud), about 20 insurance agents on a private package, sending from their own corporate domain -- see DOCS/MASOUD_PROPOSAL.md | Assessed 2026-09-08, waiting for the owner's demo. Works today: a hidden custom package, an invite code capped at 20, 20 accounts, no DNS pointing needed. To build if it closes: a per-organization sending domain verified on ACS (about two days), a billed-by-invoice mode if they want one invoice (about a day), optionally a company-level view. Condition for the deal: the ACS quota (442) granted, or a dedicated ACS email resource for this customer. |
| ~~464~~ | ~~Public package table still sold four things we do not offer (owner, 2026-09-08: 'it missed the eyes of EVERY AUDITOR')~~ **DEPLOYED 2026-09-08 in `5f12fd7` + `b9514e6`, verified at /health/version on BOTH hosts (build affb5e9); the four rows gone and the seven names live on the public table.** | Coupon manager, Need analysis calculator, Did you know manager, Get a quote form with email function -- none of the four feature codes is checked anywhere in the code. WITHDRAWN the 28-August way: definitions gone, codes added to RetiredFeatureCodes so existing databases lose the rows at startup, constants removed. Two real features were sold under names nobody recognised and are RENAMED: prospect_manager -> 'Website leads inbox (prospect manager)'; social_media_integration -> 'Social posts: draft, check platform limits, and track'. Three tests red-first. Still listed with no code check, real but ungated (left as is): Menu and sub-menu creator (the navigation editor), Built-in SEO tool, Email report and tracking system (Email Activity), Supports multilingual content, Detailed visitor tracking. **(2) 2026-09-08, owner: 'isn't it too wordy?' -- seven rows shortened to three to five words each, applied to every existing row and to the definitions: Website leads inbox; Social posts: draft and track; Website menu editor (3 levels); Built-in SEO and sitemap; Email delivery tracking; Content in any language; Website analytics. |
| ~~465~~ | ~~IDEA (owner, 2026-09-08): a social-links block for the website -- icons linking to the agent's profiles, optionally an embedded feed~~ **DEPLOYED 2026-09-08 in `8f81cfd`, verified at /health/version on BOTH hosts (build affb5e9).** | 'Social media integration' was a misnomer for Social Posts (drafting and tracking; no auto-posting, no feed). What the owner wants is a website block: profile links with icons (half a day, no third-party dependency), and possibly an embedded feed later (platform embeds are fragile and CSP-sensitive; scope separately). Not launch-blocking; the owner to decide. **BUILT 2026-09-08 (pending deploy):** a Social links block that renders the footer's social profiles in the page body (one source of truth), icons only or icons with names, in all three templates; SocialPlatformIcons shared by the footer and the block; unsafe URLs never become hrefs; a 'Social links on your website' row on every package; guide section in 04. Five tests red-first. |
| ~~466~~ | ~~Help articles open with 'In this guide', a list of links to their sections (owner, 2026-09-08: 'why didn't I think of that one?')~~ **DEPLOYED 2026-09-08 in `4029218`, verified at /health/version on BOTH hosts (build 5d09e6d).** | Cause: the new 'Social Links Block' section sat 24th in the longest guide and the article page rendered the whole guide with nothing showing its shape; the owner could not find it. HelpDocsService.RenderArticle builds the list from the level-2 headings using the ids Markdig's auto-identifier extension already gives them (a link and its heading cannot disagree), placed after the title, only when a guide has three or more sections. Three tests red-first. |
| 467 | FUTURE (owner, 2026-09-08): run the new TemplateBuilder as a 'Beta' next to the current page editor | Feasible by design: the builder exports the host's own block model (CONTRACT v0.3), never HTML, and the host renders it through its three shells, so Beta is a second way to author, not a second kind of site. Plan: (1) a per-agent flag (SuperAdmin toggle, later a package feature); (2) a 'Try the new builder (Beta)' button on My Website that hands the current site over in the export format with a short-lived signed token (no host login for the builder, no host access for its developer); (3) the host IMPORTER -- validate against the block palette and the sanitizer, preview, then publish by replacing the pages in the export -- tested on the golden fixtures requested from the builder developer on 2026-09-01; (4) the result is ordinary pages and blocks, editable in the current editor, so nobody is locked in; (5) flip the default when ready. Effort: importer 2-3 days once fixtures exist, exporter + hand-off + flag about 2 days, builder hosting as its own app service. After launch. See DOCS/TEMPLATE_BUILDER_ONBOARDING.md section 9. |
| ~~468~~ | ~~A Help icon on every portal page that opens the guide for that page (owner, 2026-09-09)~~ **DEPLOYED 2026-09-09 in `451aed5`, verified at /health/version on BOTH hosts (build 8496a90).** | HelpLinks maps controller (and action, where a screen is a section of a guide) to a help article and anchor; the topbar renders the icon with the label, new tab, aria-label. HelpLinksTests: every agent-facing controller has a target (a new screen without one fails the build), every slug exists, every anchor is a real section id in the rendered article, the layout carries the link. Gap found while mapping: no guide covers Team member logins (mapped to the account guide for now). Four tests red-first. |
| ~~469~~ | ~~Guide for Team member logins (owner, 2026-09-09) -- the gap 468 surfaced~~ **DEPLOYED 2026-09-09 in `3cef520`, verified at /health/version on BOTH hosts (build a8a223d).** | DOCS/31_TEAM_MEMBERS.md written from TeamController and the My Team page: seats per package, add (temporary password shown once, never emailed), first-login password change, reset, deactivate/reactivate/remove, what a member cannot reach (Billing, My Team). Indexed as 'team-members'; the Help icon on My Team now opens it. The index test lists it. |
| ~~470~~ | ~~Did You Know starter block arrives empty (owner, 2026-09-09: 'it does not allow me to pre-design or pre-populate the new agents')~~ **DEPLOYED 2026-09-09 in `1fbcc6c`, verified at /health/version on BOTH hosts (build 63b50ed).** | The starter block held no article choice and, even filled, could not: it stores agent Article ids, which do not exist until Resources is built, and Home is provisioned first. Now the starter block holds STARTER article ids chosen in SuperAdmin (active, this business type or All, with the teaser layout); at provisioning the agent's Articles are created first through WebsiteStarterArticleCopier (reused by title, the Resources rule), and the block is written with their real ids; the prospect preview shows the same teasers. Three tests red-first: provisioning end to end (order kept, retired starter dropped, Resources reuses the rows), the SuperAdmin save (foreign vertical and retired ids dropped), and the editor/preview pins. Guides 04 and 07. |
| 471 | Revisit: tighten what the assistant's machine can reach (owner, 2026-09-10, after the Anthropic agent-incident disclosures) | Decision item, owner-run, no code. Two zero-cost steps: (1) sign the Azure CLI out on the dev machine, or scope it to a read-only role -- domain automation runs inside the app, nothing day-to-day needs the CLI here; sign in per task if one ever does. (2) GitHub branch protection on main requiring the Actions workflow to pass, so the gate is a rule of the repository, not a habit. Standing rules already in force: no production secrets, no signing in as the owner, no restarts or config changes without an explicit go, deploys only from main after a full gate. Revisit after launch week or sooner if the owner wants. |
| ~~472~~ | ~~Client recycle bin (owner, 2026-09-10: 'a client calls and says I accidentally deleted all my clients info')~~ **DEPLOYED 2026-09-10 in `4a3b9ca`, verified at /health/version on BOTH hosts (build c0a162e).** | Until now a client delete was ClientDataEraser and nothing else: rows gone, files gone, the only way back a never-rehearsed restore of the whole production database. Now the delete snapshots every row the eraser removes (SELECT * per table in the eraser's own map, generic, no per-table code), the ids of the history rows it unlinks, and the files it would have deleted (kept), into ClientRecycleBinItems, in one transaction with the erasure. Recently Deleted page under Clients; Restore re-inserts parents before children with ids remapped, re-links history, in one transaction; nightly purge after 30 days removes the snapshot and only then the files. Erasure covers the table. Three tests red-first on real MySQL: the full round trip (comments, follow-up, life event, category link, portal document, invoice with a line, website lead re-linked; another agent cannot restore; second restore refused), the purge (expired gone with its files, fresh kept), and the wiring pins. Guides 02 and 14. |
| ~~473~~ | ~~Restore rehearsal: point-in-time restore of the production database to a throwaway server, then delete it (owner + assistant, this week, no code)~~ **DONE 2026-09-10: restore point 22:10 UTC, Ready in 7 min 14 s, counts matched production (27 clients, 2 agents), copy deleted; runbook in DOCS/14.** | Backup retention is 35 days since 2026-09-10 but the restore procedure has never been run. Half a day: `az mysql flexible-server restore` to a new server, connect, confirm the schema and a few rows, write the runbook into DOCS/14, delete the server. Already planned as a pre-launch item in the launch-runway notes; now scheduled. |
| ~~474~~ | ~~Nightly logical dump of the database to blob storage, 30-day retention~~ **DEPLOYED 2026-09-11 in `161fac8`, verified at /health/version on BOTH hosts (build ed2bd8b); the owner moved it before launch on 09-10.** | DatabaseDump writes the script in-process (no mysqldump on Linux App Service): every base table's SHOW CREATE TABLE and its rows as batched INSERTs, values escaped like the mysql client, Hangfire tables excluded, foreign-key checks off around the replay. DatabaseDumpJob gzips it to the private db-backups container nightly at 06:15 UTC and removes dumps older than 30 days by the timestamp in the name; SuperAdmin Backups page lists the files and runs a dump on demand (audit-logged). Three tests red-first on real MySQL: the dump replays into an empty database with every value intact (quotes, backslashes, newlines, emoji, dates, decimals, booleans, nulls, a bigint), the job uploads a gzipped private file with the right name and removes only the 31-day-old dump, and the wiring pins. Restore runbook in DOCS/14; guide 07. A file we own, restorable table by table, independent of Azure's own backups: a Hangfire job (or an Azure automation) runs mysqldump to a private container with a lifecycle rule. Decided 2026-09-10 with the owner as step 4 of the data-backup plan; after launch. |
| ~~475~~ | ~~Starter Articles editor: add from a group, and a Business Type list (owner, 2026-09-09: could not add an article under Mortgage)~~ **DEPLOYED 2026-09-10 in `9137190`, verified at /health/version on BOTH hosts (build d632b69).** | The add button sat at the top of a long grouped list, Business Type was a free-text box (a typo files the article under a vertical of its own) and nothing pre-filled. Now every group header has 'Add to this group' (business type and category filled in, next sort order), and Business Type is a datalist backed by StarterBusinessTypes: All, the three shipped verticals, then every value already in use across starter pages, articles, forms and agents (trimmed, case-folded), so a new vertical typed once is offered from then on. Same list on the starter page form. Three tests red-first (the list, the pre-filled Create with next sort order, the view pins). Guide 07 gained a Starter Articles section. |
| ~~476~~ | ~~Backups page: a Size column per dump (owner, 2026-09-11, after the first on-demand dump: 'so we could notice if there is a huge swing')~~ **DEPLOYED 2026-09-11 in `ee59468`, verified at /health/version on BOTH hosts (build 1a1bbb0).** | IBlobStorageService.ListAsync returns URLs only; add a listing method that returns name, size and created time (the SDK's BlobItem.Properties.ContentLength / CreatedOn), show the size per row (KB/MB) and, when there are two or more dumps, the change against the previous one so a swing stands out (a colour when it is more than about 25%). Fake store in DatabaseDumpTests grows the same method. About 45 minutes with tests; no deploy of its own. |
| **477** | **LAUNCH: point www.iproaccountants.com and www.iproadvisers.com (and both apexes) at the new site, app.iproadvisers.com (owner, 2026-09-11) -- code DEPLOYED 2026-09-11 in `417da98`, verified at /health/version on BOTH hosts (build deec85e); the DNS switch is scheduled on or before 21 September 2026 (owner, 09-11); iproaccountants.com lands on /accountants (478) once that page is built from the designer's package; runbook in DOCS/14** | Today all four names resolve to 66.102.128.65 (the old site, serving 200 over HTTPS); app.iproadvisers.com is a CNAME to ipro-prod-web.azurewebsites.net. Plan (recommended): a 301 redirect of every request on the old names to https://app.iproadvisers.com/, HTTPS kept working on the old names. Code (assistant, 1-2 h, deploy BEFORE the DNS switch): App:AliasHosts in configuration and a middleware that 301s any request whose host is an alias to the canonical host (home page, not path-preserving, so old paths do not land on 404s); tests. Azure (owner's go): bind the four hostnames to ipro-prod-web with App Service managed certificates (each needs a TXT asuid.<host> verification record first). DNS (owner, at the registrar, on the day he chooses): www -> CNAME ipro-prod-web.azurewebsites.net; apex -> A 40.89.19.0 (the app's inbound IP) plus the TXT records; touch NOTHING else on iproadvisers.com -- the SPF/DKIM/MX records carry the ACS email domain. After the switch: verify the four names 301 to the platform home over HTTPS and that mail still authenticates. Decision for the owner: redirect (recommended) versus serving the platform under the old names. **ipromortgages.com done 2026-09-12 (483):** bound, certificate issued, alias set; on the switch day the four live names are added to the existing `App__AliasHosts` value. |
| **478** | **LAUNCH: vertical landing pages inside the platform -- /accountants DEPLOYED 2026-09-11 in `fc8fd00`, verified at /health/version on BOTH hosts (build 420764a); /mortgage DEPLOYED 2026-09-11 in `770d126`, verified at /health/version on BOTH hosts (build 1c80f68); /insurance follows the same template (owner, 2026-09-11)** | Decided with the owner: three pages on the one site beat three separate sites (one codebase, one search authority, nothing to keep in sync); the old domains redirect to their own page (477), so each domain keeps addressing its audience. The designer works from DOCS/VERTICAL_PAGE_BRIEF_ACCOUNTANTS.md (sections, audience, content sources, palette, widths, the links to wire: Register?businessType=Accountants, which already pre-selects, and Preview/Show?businessType=Accountants). Build from the approved design, about a day per template, the other two verticals as variants. Owner-side: register ipromortgages.com (cheap, protects the name) so it can join the redirect list. **/accountants (09-11):** package accountants-page-v2-font-revision checked against the acceptance list -- desktop clean, phone width clipped the hero by 26 px (a 1fr grid track sized to a 116%-wide device stage; fixed in the platform copy of the stylesheet, marked 478), buttons 51 px, no missing images, copy matched. Built as HomeController.Accountants + Views/Home/Accountants.cshtml (Layout = null like the home); stylesheet and icons moved to the shared places below with the second page. Slots: the designer's header and footer carry the platform logo, Sign in, Terms and Privacy; the pricing slot is cards from the same public-package query as the home (extracted to LoadPublicPackagesAsync) with a link to the home's full comparison; the preview slot is the live accountant starter site in a same-origin iframe (/Preview/Site?businessType=Accountants). Renders for signed-in agents too. The short address is an attribute route (the conventional route read /accountants as a controller and answered 404 on the first local render; the test pins the route now). Four tests red-first. **/mortgage (09-11):** package mortgage-page-v1 (the designer's corrected four-column footer) checked the same way -- desktop clean, phone width clean (the designer carried the fix: the hero is a block at phone width, the minmax(0,1fr) track above it), copy matched, icons identical to the accountants set. Built as HomeController.Mortgage (routed /mortgage, and /mortgages because the README named it that) + Views/Home/Mortgage.cshtml on the same template. What the second page changed for both pages: one stylesheet, wwwroot/css/landing-page.css (accountants-page.css removed), the icons under images/landing, and two partials -- Views/Home/_LandingFooter (the corrected footer: the home's sections, the vertical pages, the same social links as the home, Privacy and Terms at the live /privacy and /terms; the first cut of /accountants linked /Home/Terms and /Home/Privacy, which answer 404 in production, caught while checking the mortgage package's links) and _LandingPricing (the cards, register links carrying ViewData["BusinessType"]). Five tests red-first, the accountants tests re-pinned to the shared files. Once ipromortgages.com is registered and bound, App:AliasHosts gains www.ipromortgages.com=/mortgage,ipromortgages.com=/mortgage (runbook in DOCS/14). Next vertical: /insurance needs only its own sections from the designer; footer, pricing and stylesheet are shared already. |
| **479** | **Did You Know teasers: entities decoded; the 2 x 3 grid holds inside the landing pages' preview frame -- DEPLOYED 2026-09-11 in `843f4be`, verified at /health/version on BOTH hosts (build 403838f) (owner, 2026-09-11)** | The owner, checking /accountants: make the Did You Know 3 x 2 for both pages. The layout exists per block (Grid, 2 rows x 3 columns: SuperAdmin -> Starter Content -> the vertical's Home -> Did You Know -> Layout; agents set their own under Website Pages -> Edit -> the block); what stopped it showing was the stylesheet: the stack rule sat in the 800 px phone block of _ManagedPageStyles, and the landing pages' preview frame is about 620 px wide on a desktop, so the grid collapsed to one column inside it. Now only screens under 560 px stack it. Seen in the same frame: a teaser reading a literal "&mdash;" -- DidYouKnowBuilder stripped the tags from the article body but left its HTML entities in, and the page encodes the excerpt again; the excerpt decodes them now. Two tests red-first. The owner had already set Grid with six articles on both the Accountants and Mortgage starter blocks (confirmed on the live previews: dyk-grid-2x3, six teasers each); the same fetch showed the literal entities on both (&mdash; on Accountants, &nbsp; on Mortgage). |
| **480** | **Reply-To is the adviser's own address on every client-facing message, webmail or not -- DEPLOYED 2026-09-12 in `e8ed964`, verified at /health/version on BOTH hosts (build c61bf13) (owner, 2026-09-12)** | The owner's drip test on 09-12: the message arrived From IPRO Advisers with Reply-To support@iproadvisers.com, so a client's reply to their adviser lands at support, which would have to be watched around the clock and relayed by hand. Cause: 440 (09-01) replaced a free-webmail Reply-To (Gmail, Yahoo, Hotmail, Outlook.com, iCloud, Proton...) with the support address at the provider seam to avoid SpamAssassin's FREEMAIL_FORGED_REPLYTO (+2.5 on receivers that run it; mail-tester 7.7 vs 10). Decided with the owner: a reply that reaches the adviser outweighs a partial spam score that only SpamAssassin receivers apply (the owner's cPanel sample would move from 0.5 to about 3.0 against a 5.0 threshold; Gmail and Microsoft do not use that rule). Both providers now keep the caller's Reply-To as given; the support address is only the fallback when none is given; the FreemailDomains classifier and its tests are gone (history in 440 and `20b9cd0`). Tests red-first (AdviserReplyToTests replaces FreemailReplyToTests). Later, separately: the From display name as '<adviser> via IPRO Advisers' (ACS fixes the display name per sender address, so it needs a sender per adviser or a shared 'via' address), and after launch a forwarding alias per adviser on our domain, which would give the clean header shape and direct replies at once. |
| **481** | **E-letter and e-card creation: the send and its recipient rows commit together; recipients stranded Queued under a finished send are marked Failed at startup -- DEPLOYED 2026-09-12 in `5fbcd8b`, verified at /health/version on BOTH hosts (build 26182bc) (owner, 2026-09-12)** | The owner's e-letter to two clients (09-12, 9:15): the first delivered, the second Queued with no Sent time, still Queued at 9:50. Read-only evidence: ACS's own metric shows exactly one SendMail for it and none after (no retry ever ran); the production container log has no error and no warning from the dispatcher (a crash or an abandoned run would have logged one); the dispatcher mails every Queued row it finds (probed on 09-12 both the button's way, rows tracked on the same context, and the job's way). So the second row was not there to find: ELettersController saved the letter first -- Scheduled with ScheduledAt = now, already DUE for the minutely dispatch job -- and the recipient rows in a second save. The job can claim the letter in that gap and finish with none, or one, of its recipients; the letter ends Sent (one sent), the rest stay Queued under it forever, and the button's own dispatch then finds nothing to claim (claim-first), while the page still says 'sent to 2'. ECardsController had the same shape. Newsletters and polls build their rows inside the dispatcher, so they never had the gap. Fix: one transaction around the parent and its rows in both controllers, so nothing sees the send before its rows exist. Repair: StartupSchemaRepair.RepairRecipientsStrandedUnderFinishedSendsAsync marks Queued rows under a Sent or Failed letter or card as Failed with the reason on the row ('...finished before this recipient was picked up... Send it again.'), in both apps at startup, idempotent; rows under Scheduled or Sending parents are left alone. Tests red-first (source order pins in both controllers, the repair's behaviour, both Program.cs). Owner-side after the deploy: the stuck row shows Failed with that reason; send the letter to that client again. |
| **482** | **SuperAdmin behind Microsoft Entra sign-in -- DONE 2026-09-12 (owner in the portal, health paths excluded from the CLI on the owner's go). Client secret expires 2028-09-11: rotate it in the portal before then (owner, 2026-09-12)** | The owner's question of 09-11: the office has no static address, so an IP allow-list on the admin site would lock him out whenever the address changed. Chosen instead: App Service Authentication (Easy Auth) with the Microsoft identity provider in front of `ipro-prod-admin` (resource group `ipro-prod-admin_group`, host admin.iproadvisers.com), the SuperAdmin username/password login staying behind it as the second gate. Owner, portal: Authentication -> Add identity provider -> Microsoft; new app registration `IPRO SuperAdmin sign-in`, Workforce (current tenant), single tenant, client secret 24 months; Client application requirement = this application only; Identity requirement = any identity; Tenant requirement = issuer tenant only; Require authentication; unauthenticated requests -> HTTP 302 to Microsoft; token store on. Assistant, CLI on the owner's go (the portal dialog has no field for it): `az webapp auth update -n ipro-prod-admin -g ipro-prod-admin_group --excluded-paths "/health,/health/version"` (one comma-joined argument; under Git Bash prefix `MSYS_NO_PATHCONV=1` or the paths become Windows paths) -- the deploy workflow's verify step and the host watch read `/health/version` anonymously and were answering 401 until this landed (about 60 s to take effect). Verified: `/health` and `/health/version` 200 for anyone with the build SHA; `/`, `/Admin/Login`, `/hangfire`, static files -> 302 to login.windows.net for a browser, 401 for API-style callers; the web host untouched. Assignment required = Yes on the enterprise application with the owner assigned, so only he passes the first gate. Runbook in DOCS/14. |
| **483** | **ipromortgages.com registered, bound and live on /mortgage (redirecting at first; served in place since 484 the same afternoon) -- DONE 2026-09-12, the dress rehearsal for the 21 September switch (owner, 2026-09-12)** | The owner registered ipromortgages.com at GoDaddy on 09-12 (item 478's owner-side note). Because nothing ran on the name, the whole domain-switch runbook (DOCS/14, 477) was exercised on it the same afternoon: owner at GoDaddy -- A `@` = 40.89.19.0, CNAME `www` = ipro-prod-web.azurewebsites.net, TXT `asuid` and `asuid.www` = the app's custom-domain verification id (GoDaddy's own NS, SOA, _domainconnect and _dmarc rows left alone; the pre-created `www` CNAME edited rather than duplicated, since a name takes one CNAME); assistant on the owner's go -- `az webapp config hostname add` for both names (Verified at once, the records were already visible at GoDaddy's authoritative servers and at 8.8.8.8), `az webapp config ssl create` for the two App Service managed certificates, then `App__AliasHosts` = `www.ipromortgages.com=/mortgage,ipromortgages.com=/mortgage` on ipro-prod-web (one restart). Verified from outside: both names answer 301 to https://app.iproadvisers.com/mortgage over a valid managed certificate. Seen on the way: before the alias setting the app answered the new host with its 'Website not published yet' page (an unknown host is treated as an agent site), which is the expected in-between state. On the 21st the four live names follow the same recipe, and the alias setting is APPENDED to (DOCS/14 step 3 updated), never overwritten. |
| **484** | **A brand domain serves its landing page under its own name (ipromortgages.com shows /mortgage; iproaccountants.com will show /accountants) -- DEPLOYED 2026-09-12 in `59c8c78`, verified at /health/version on BOTH hosts (build aa56ef5) and live at https://www.ipromortgages.com (owner, 2026-09-12)** | The owner, the minute 483 went live: typing https://www.ipromortgages.com must not bounce to app.iproadvisers.com/mortgage; the brand name stays in the address bar showing that page. PlatformAliasHosts.TryHandle is now the one rule the pipeline calls first: an alias WITH a landing path is a brand domain -- its root is re-addressed to the platform host and the landing path and carries on down the pipeline (so routing, the packages, the preview iframe all behave as on the platform), and anything with a file extension or under /Preview/Site (the page's stylesheet, icons, favicon, the live preview frame) is served the same way; every other path on the brand name (Register with its business type, Sign in, the preview pages, Terms, Privacy, an old deep link) is a permanent redirect to the platform WITH its path and query, because accounts and the rest of the site live there. An alias WITHOUT a landing path (the iproadvisers.com pair) keeps redirecting to the home, files included: it is the platform's own brand. /.well-known passes untouched on every name. The landing partials' links to the home's sections became absolute (`@platformBase/#i2-...`): on a brand name `/#i2-pricing` would have been the brand's own root. The page's canonical tag still names the platform address, so search keeps one authority (the owner's 09-11 call; he can flip it to the brand name later if he wants ipromortgages.com itself in results). Tests red-first (the in-place set, root and files and frame re-addressed, the rest redirected with path and query, the iproadvisers names still redirecting, platform and agent hosts and /.well-known untouched, a local base keeping its port, the wiring before routing, the absolute home links); the 477 wiring pin moved into the new test. |
| **485** | **Google Calendar connect failed with redirect_uri_mismatch on every host since 2026-08-07 -- DEPLOYED 2026-09-12 in `4fe3f37`, verified at /health/version on BOTH hosts (build c490473) (owner, 2026-09-12)** | The owner's test on 09-12 from an agent domain, the platform host and the temporary domain: Google's 'Access blocked: redirect_uri_mismatch' every time. Connect and Callback built the OAuth redirect_uri with Url.ActionLink, which reflects the route table; since the portal got its own URL space on 2026-08-07 (the `portal` route registered ahead of `default`) that produced https://app.iproadvisers.com/portal/GoogleCalendar/Callback, and Google only has the unprefixed address registered. The 07-28 fix (the canonical-host bounce) was still doing its job; the path under it had moved. Nobody had connected a calendar in production in between. Fix: PortalUrlHelper.GoogleCalendarRedirectUri = the canonical base + the literal `/GoogleCalendar/Callback`, used by Connect and by the code exchange in Callback, and Callback carries `[HttpGet("/GoogleCalendar/Callback")]` so it answers at exactly that path whatever the conventional routes do. Tests red-first (the address from configuration in three shapes; the source using it in both places and not Url.ActionLink; the route on the action). **Owner-side:** the Google Cloud console's OAuth client must list `https://app.iproadvisers.com/GoogleCalendar/Callback` (DOCS/09 named only the azurewebsites.net one); check Credentials -> the OAuth 2.0 client -> Authorized redirect URIs before retesting. |
| **486** | **Google Calendar asks for calendar.events, not the full calendar scope; the OAuth verification runbook -- DEPLOYED 2026-09-12 in `e3f5a27`, verified at /health/version on BOTH hosts (build 577589b) (owner, 2026-09-12)** | With 485 deployed the owner reached Google's consent step and saw 'Google hasn't verified this app -- an app that's currently being tested': the consent screen is in Testing. He asked to do the verification now rather than Monday. Code: the sync only lists, inserts, updates and deletes events on the adviser's chosen calendar, so the app asks for `calendar.events` + `userinfo.email` (the full `calendar` scope covered settings, sharing and every calendar and was never needed; verification is per scope). Test red-first on the authorization URL. Owner-side, in DOCS/27: add `calendar.events` on the console's Data Access page BEFORE this deploys (Google refuses a requested scope the page does not list -- the July incident), Branding fields (home, privacy, terms on app.iproadvisers.com; authorized domain iproadvisers.com proven in Search Console by a TXT record), publish to production, submit for verification with the justification text and the demo-video script from DOCS/27. Bridge: published but unverified, an adviser connects through 'Advanced -> Go to IPRO Advisers', capped at 100 users. |
| **487** | **Google Calendar connect refuses a grant without calendar access, with a plain message -- DEPLOYED 2026-09-12 in `1c0c591`, verified at /health/version on BOTH hosts (build ced124b) (owner, 2026-09-12)** | The owner reconnected on the 486 build and every sync run then failed with Google's 403 'Request had insufficient authentication scopes' (read in the container log): Google's consent screen shows the calendar permission as a box the person can leave unticked, the token came back with only the email, and the connection was stored as a success. Reconnecting with the box ticked fixed it. Now GoogleCalendarService.ExchangeCodeAsync reads the `scope` Google returns with the token and, when neither calendar.events nor the older full calendar scope is in it, throws with 'Google did not grant access to your calendar. Connect again and, on Google's screen, tick the box for your calendar events before you allow.' -- which the Callback already shows as the error banner -- and nothing is stored. Tests red-first with a fake Google (the classifier; a consent without the box refused with that message; a consent with it connecting). DOCS/27: the checkbox is a known trap and the demo video ticks it on camera. |
| **488** | **Platform-owned open and click tracking for the six marketing channels -- DEPLOYED 2026-09-14 in `fa58c71`, verified at /health/version on BOTH hosts and live at /t (build 7c19d76) (owner, 2026-09-14)** | Microsoft will not enable engagement tracking on a custom domain at default sending limits (442: declined 09-04 on volume, under review since 09-10, the owner's 09-11 split request and the 09-14 nudge for a decision date unanswered; the limits page updated March 2026 still says so), and the launch is on the 21st. So the platform does it: `EmailTrackingLinks` appends a one-pixel image and rewrites absolute http(s) links through a signed redirect on the platform host (`/t/o/{kind}/{token}.gif`, `/t/c/{kind}/{token}?u&s`), keyed by a per-recipient `TrackingToken` (24 random bytes, base64url) that each dispatcher mints just before the send and stores on the row -- newsletters, drip steps, e-cards, e-letters, polls, Did You Know; column and index added by `EmailDeliverySchema` at startup. `EmailTrackingController` records `open`/`click` through the SAME two recorders the ACS webhook feeds (NewsLetterService for newsletters and drip steps, EmailDeliveryTracker for the rest), so OpenedAt/ClickedAt, the roll-ups and Email Activity are unchanged; invoice mail stays 'not tracked' (the invoice page stamps its own views). Safeguards: the redirect target is HMAC-SHA256-signed (key `Email__TrackingSigningKey`, falling back to the Event Grid webhook secret already on the web app; no key = pixel only, links direct); unsubscribe/preferences, vote and every `token=` link, mailto:, tel:, anchors and img src are never rewritten; an unknown or empty token gets the image and records nothing; `/t` reserved in IsNeverShadowedPrefix; Cache-Control no-store. Switch: `Email__PlatformTrackingEnabled=false` on BOTH App Services -- the choice if Microsoft later enables provider tracking, so the two do not stack (DNS_ZONE_RUNBOOK). The plain-text parts are untouched. Tests red-first (29): the token, the pixel, rewriting and its exclusions, the signature, the settings, the endpoints against a real database (card open, unknown/empty token, signed click, refused click, newsletter roll-up), and wiring pins on the six dispatchers, Program.cs, the schema and the screens. Privacy: the policy already says 'opened'; 'clicked' goes in with 489. DOCS/18 and DOCS/23 corrected. Owner's confirmation, same evening: a card to the QA mailbox delivered 17:52, opened 17:52 when the images were allowed, clicked 17:53, status Clicked on Email Activity; inbox, no spam tag; the mail client asked to approve images, as it does for any sender not in the address book. |
| **489** | **Privacy policy caught up with three facts: clicks are measured by the platform (488), the Google Calendar paragraph Google's OAuth reviewer reads, Azure Communication Services as the deliverer -- DEPLOYED 2026-09-14 in `bbd5b6a`, verified at /health/version on BOTH hosts and live at /privacy (build 0f06831) (owner, 2026-09-14)** | Found while preparing the Google verification (DOCS/27) and building 488. Both copies changed the same way -- the rendered partial `_LegalPrivacy.cshtml` and the reviewer's Markdown source `DOCS/legal/privacy-policy.md` -- and `PrivacyPolicyTests` (red first, 9 pins) keeps them in step; `DOCS/legal/README-review-notes.md` gained a dated entry for counsel. Last updated moved to 14 September 2026. (1) Email delivery and engagement: 'delivered, bounced, opened, clicked or reported as spam', and how -- a small image that loads on open and links that pass through our own address; not sold, not used for advertising. (2) New 'Your Google Calendar, if you connect it' under section 2: what is asked for (events on the one chosen calendar, the account email), the single purpose (two-way sync every fifteen minutes), what is not done, storage (encrypted, database in Canada), deletion (Disconnect revokes with Google and deletes the row; closing the account too), and the Google API Services User Data Policy / Limited Use sentence. (3) Section 4 table: Microsoft Azure Communication Services delivers the mail (data location Canada, read from the resource); SendGrid listed as standby (it stays in the codebase as the rollback provider); the Google row points at section 2. Nothing here changes what the platform does; the lawyer's review of both documents is still outstanding (README-review-notes). |
| **490** | **Google Calendar Disconnect also removes the events copied in from Google; the Calendar page shows copies only while connected -- DEPLOYED 2026-09-15 in `4775ff0`, verified at /health/version on BOTH hosts (build 296a6c1) (owner, 2026-09-15)** | The owner disconnected after recording the verification video and found the Google events still on /portal/Clients/Calendar: Disconnect revoked and deleted the credentials but left the ExternalCalendarEvents copies, and the confirm text said so ('Events already synced will stay as-is'). Now GoogleCalendarController.Disconnect removes the agent's ExternalCalendarEvents in the same SaveChanges as the connection (revoke failure or not; whether or not a connection row was found, so pre-490 leftovers go too) and the success message says how many; ClientsController.Calendar loads copies only when a GoogleCalendarConnection exists (hides pre-490 leftovers immediately); the confirm text says copies are removed, follow-ups stay, and the events on Google stay there. Follow-ups keep their GoogleEventId so a reconnect does not push them to Google twice. Tests red-first (4): the removal with the other agent's copies and the follow-up untouched and the revoke token seen; the removal when Google refuses the revoke; the Calendar page with and without a connection; the confirm-text and controller pins. Consistent with the 489 policy paragraph (credentials revoked and deleted on disconnect) and with what Google's reviewer watches at step 8 of the video. |
| ~~459~~ | ~~My Website: collapse the two registrar-setup cards once the primary domain is fully connected (owner, 2026-09-02)~~ **DEPLOYED 2026-09-02 in `2b426c2`, verified at /health/version on BOTH hosts (build 9ea67ac).** | When Connection status already reads Found / Connected / Secured / Forwarding OK, steps 1 (CNAME) and 2 (forwarding) take half the page for nothing. Keep them, collapsed, behind 'Show the setup steps (for adding another domain)'; expanded as today while anything is still pending or there is no custom domain yet. Owner's words: 'the information is there but not as taking half the page'. **BUILT 2026-09-02 (pending deploy).** DomainSetupState.IsFullyConnected; three tests red-first. |
| ~~460~~ | ~~The appointment emails name the client portal but give no address (owner, 2026-09-02, from the 'Your appointment has been scheduled' email)~~ **DEPLOYED 2026-09-02 in `8c14f1e`, verified at /health/version on BOTH hosts (build 9ea67ac).** | 'You can review this anytime from your client portal' now links to the sign-in page: https://<their-domain>/ClientPortalAccount/Login when a custom domain is attached and live, otherwise https://app.iproadvisers.com/ClientPortalAccount/Login -- the owner's rule, now the resolver's rule (457 adjusted: the free subdomain is no longer a tier). The declined email offers the portal for a new request. Three tests red-first on the real Schedule / Decline actions. **BUILT 2026-09-02 (pending deploy).** |
| ~~461~~ | ~~The in-app Help Documentation (/portal/Support) had fallen behind (owner, 2026-09-02: 'you guys did not update the support documentation')~~ **DEPLOYED 2026-09-02 in `2f1415a`, verified at /health/version on BOTH hosts (build 9ea67ac).** | Checked 2026-09-02. Two kinds of gap. (a) Written but not listed: 17_FORMS, 18_ECARDS, 19_ELETTERS, 21_AGENT_GOOGLE_VISIBILITY existed on disk and were never added to HelpDocsService / the embedded resources -- FIXED, four new articles in the index (pinned by HelpDocsTests). Today's features written into the guides: invoice delivery tracking and Resend (10), step 1 at enrolment and the Campaigns tab (03), the collapsing setup steps (05), the portal address, invite-email status and appointment links (11). (b) No guide at all yet for: Email Activity, Marketing Calendar, the AI Daily Assistant, Did You Know, Support tickets themselves, Calendar / Google Calendar, Articles / blog, the page editor's Image Library. Each is a short how-to; write them from the real screens (an hour or two for the set). Standing rule from here: a feature is not done until its guide is updated -- the help index test lists every agent-facing DOCS file. **SECOND HALF BUILT 2026-09-02 (pending deploy):** eight guides written from the real screens (23 Email Activity, 24 Marketing Calendar, 25 AI Daily Assistant, 26 Did You Know, 27 Support and Help, 28 Calendar and Google Calendar, 29 Articles, 30 Image Library), indexed and embedded; the index test lists all 24 agent-facing guides. **SECOND HALF DEPLOYED 2026-09-02 in `2664b6b` (build 3948647): 24 agent-facing guides in the Help index, all pinned.** |
...
| ~~433~~ | ~~ACS email piece 2: Event Grid delivery/bounce pipeline~~ **BUILT 2026-08-31** | Replaces NewsletterController.SendGridEvents: subscribe the Email Communication Service to Event Grid (EmailDeliveryReportReceived + engagement events), correlate on the operation id already stored as ProviderMessageId, feed the SAME three consumers (newsletter recorder, EmailDeliveryTracker, EmailConsentService suppression). Until then bounces/complaints are not recorded (CASL suppression only via our own unsubscribe links, which are provider-independent and still work). ALSO before launch: check the new ACS resource's sending quota and request an increase if newsletters would exceed it. **SHIPPED:** AzureEmailEventsController receives Event Grid, completes the subscription-validation handshake, authenticates on a shared secret (Email__AzureEventWebhookSecret, fixed-time compare), correlates ACS's messageId against the ProviderMessageId every dispatcher already persists, across ALL SIX senders (newsletter, drip -- joined through the enrollment for its ClientId -- e-card, e-letter, poll, DidYouKnow), and feeds the SAME three consumers. Status map: Delivered->delivered, Bounced->bounce, Suppressed/Failed/Quarantined/FilteredSpam->dropped, Expanded->processed, View->open, Click->click; an unrecognised status is SKIPPED, never coerced. 24 tests, 16 observed RED first. **KNOWN GAP, PERMANENT: ACS emits NO spam-complaint and NO unsubscribe event** -- two event types, seven statuses, none of them a complaint. Under SendGrid, "mark as spam" in Gmail suppressed the client across every channel; that path is GONE and cannot be rebuilt on ACS. Our own unsubscribe links are provider-independent and still carry consent, so the legal path is intact. **COMPENSATION (owner-approved):** a HARD BOUNCE now suppresses across all channels, which nothing did before -- a months-long bounce rate is what got the SendGrid account terminated. Quarantined and FilteredSpam deliberately do NOT suppress: those are the receiving organisation's filter, not the person's choice. **OWNER, BEFORE LAUNCH:** (1) create the Event Grid subscription on ipro-prod-comms -> endpoint https://app.iproadvisers.com/AzureEmailEvents?secret=<value>, event types EmailDeliveryReportReceived + EmailEngagementTrackingReportReceived; (2) set Email__AzureEventWebhookSecret to that same value on BOTH App Services; (3) still outstanding -- check the ACS sending quota. |
| ~~432~~ | ~~Admin header clock shows UTC, portal shows agent-local~~ **DEPLOYED 2026-09-09 in `06f309b`, verified at /health/version on BOTH hosts (build 220a1d2): AdminClock renders Admin:TimeZone (Eastern when unset) with a short label.** | Cosmetic: Admin's header timestamp read "8:42 PM" when local (Eastern) time was ~4:42 PM — it renders UTC. Portal lead timestamps are correct agent-local. Low priority; fix = format the Admin header with a configured admin timezone. |
...
- **434 — Website Leads: no way to remove a lead + dismiss UX reads as broken.** Found 2026-08-30
  ...
  like a failed delete. Real needs: (a) hard-delete for junk/spam/test leads — also a PIPEDA
  consideration (non-client personal data retained forever with no removal path); (b) either
  exclude dismissed from "All" or label the tab honestly. Not launch-critical: Dismiss covers the
  daily workflow.
...
12. ~~signup says "Verify code is incorrect" when the code is right~~ — **FIXED 2026-08-20**
   (branch `fix/medium-sweep`): the expired-session case gets its own honest message ("your session
   timed out, the form was refreshed with a NEW code") and the very next attempt succeeds, which
   also defuses the 5/hour lockout spiral. Source-walk test.
   ORIGINAL: **NEW DEFECT 2026-08-18 — signup says "Verify code is incorrect" when the code is right.** The
...
13. ~~possible gap recording the setup-fee transaction id~~ — **VOID, owner-verified 2026-08-20**:
   invoice IPRO-2026-000010 now lists the subscription id AND BOTH sale transaction ids
   (9V157229VL199150M + 3B289037K78907255) — the absorb rule appended the second sale when it
   settled. The earlier single-id snapshot was mid-flight, the same timing artifact as the
   $169.50-vs-$214.70 scare. Nothing to fix; the replay-dedupe guard has both ids to match on.
   ORIGINAL: **UNCONFIRMED 2026-08-20 — possible gap recording the setup-fee transaction id.** At 12:51 the
...
CACHE404, PARENT, EMPTYHOST, RESEND, JOBS-5/6/7/8/10, JOBISOLATION, M-8 claim-early+release,
STARTER, ADMIN-6, ADMIN-8, QUOTA+DOCUSAGE, ERASEATOMIC), **2 verified already fixed** (JOBS-9 by
LB-2; #410 by the seeder repair — both stale entries), **1 disposition** (#394: the recording
mechanisms that could have missed those charges are all closed since — webhook txn-id dedupe
`8dcc6d3`, the absorb rule `f4424fd`/`1dfacdb`, hourly PayPal reconcile; the historic instance
stays unexplained and unexplainable without PayPal-side data from that week, recorded as such),
and **1 awaiting the owner** (SETUPFEE-TXN: re-open BobyMot invoice IPRO-2026-000010 and check the
PayPal column now lists BOTH txn ids — 10 seconds, voids or confirms the item). 13 new tests.
