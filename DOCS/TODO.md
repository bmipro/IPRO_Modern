...
| 402 | QA restart **day 2** (Aug 11): confirm overnight charge → upgrade **QA Gold (Daily), id 8** | Overnight sale ~20:45 ET should produce ONE invoice: **$40.00 + $5.99 QC = $45.99** (first test of both the de-tax fix and the duplicate-absorb fix on a clean cycle). Then on /portal/Billing: console override `document.querySelectorAll('input[name="billingRuleId"]').forEach(i => i.value = 8)` → Upgrade Monthly. Verification: **old Silver sub CANCELLED at PayPal** (the #396 supersede path, finally under observation), new Gold sub ACTIVE daily, upgrade invoice at Gold prices with QC tax. |
| **405** | **Tax-rate divergence: plan pricing 14.98% vs invoicing 14.975%** | PayPal plan amounts imply a QC rate of 14.98% ($172.47, $45.99 = ×1.1498) while invoices compute at 14.975% — one screenshot even shows both on the same page ("QC 14.975% GST+QST 14.980 %"). Source of the stray cent in the $150.01 line. Somewhere the rate is stored/rounded to 4 decimal places. Find where the plan build gets its rate and make both sides read the same number. Cosmetic-cent today; wrong-by-design for accounting. |
