...
| **472** | **Client recycle bin (owner, 2026-09-10: 'a client calls and says I accidentally deleted all my clients info') -- BUILT 2026-09-10 (pending deploy)** | Until now a client delete was ClientDataEraser and nothing else: rows gone, files gone, the only way back a never-rehearsed restore of the whole production database. Now the delete snapshots every row the eraser removes (SELECT * per table in the eraser's own map, generic, no per-table code), the ids of the history rows it unlinks, and the files it would have deleted (kept), into ClientRecycleBinItems, in one transaction with the erasure. Recently Deleted page under Clients; Restore re-inserts parents before children with ids remapped, re-links history, in one transaction; nightly purge after 30 days removes the snapshot and only then the files. Erasure covers the table. Three tests red-first on real MySQL: the full round trip (comments, follow-up, life event, category link, portal document, invoice with a line, website lead re-linked; another agent cannot restore; second restore refused), the purge (expired gone with its files, fresh kept), and the wiring pins. Guides 02 and 14. |
