...
| 401 | QA restart **day 1**: fresh signup → QA Silver (Daily) | After signup, tell the assistant "done" — verification at PayPal's API + App Insights within a minute: invoice $40.00 + $5.20 HST = **$45.20 total** (not $51.08), webhook 200, sub ACTIVE. |
| 402 | QA restart **day 2**: confirm overnight charge → upgrade **QA Gold (Daily), id 8** | The invoice must be right at Gold's price AND **the old Silver sub must go CANCELLED at PayPal** — this is the #396 supersede path under observation. |
