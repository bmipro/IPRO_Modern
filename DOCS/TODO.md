...
| 458 | FUTURE: agent-level error pages -- on an agent's own domain, a public-site 404 in the site's own design (header, colours, navigation), and optionally a per-website '404 page' block the agent can write | Deferred by the owner on 2026-09-02 in favour of the platform-branded pages (456). The recipe is in DOCS/ERROR_PAGES.md ('Later: agent-level pages'): render a NotFound view through the managed-page shell with Response.StatusCode = 404 where PublicWebsiteController returns NotFound() today -- the status-page middleware only fires on a bodyless response, so a themed 404 simply wins; keep the platform page for the agent portal, client portal and sign-in. Post-launch unless an agent asks. |
