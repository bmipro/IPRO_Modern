...
| 473 | Restore rehearsal: point-in-time restore of the production database to a throwaway server, then delete it (owner + assistant, this week, no code) | Backup retention is 35 days since 2026-09-10 but the restore procedure has never been run. Half a day: `az mysql flexible-server restore` to a new server, connect, confirm the schema and a few rows, write the runbook into DOCS/14, delete the server. Already planned as a pre-launch item in the launch-runway notes; now scheduled. |
