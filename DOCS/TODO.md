...
| ~~432~~ | ~~Admin header clock shows UTC, portal shows agent-local~~ **DONE 2026-09-09 (pending deploy): AdminClock renders Admin:TimeZone (Eastern when unset) with a short label.** | Cosmetic: Admin's header timestamp read "8:42 PM" when local (Eastern) time was ~4:42 PM — it renders UTC. Portal lead timestamps are correct agent-local. Low priority; fix = format the Admin header with a configured admin timezone. |
