...
| ~~398~~ | ~~BILLING -- audit ALL live sandbox subscriptions~~ | **CLEAN, 2026-08-12** (one buyer-side confirmation outstanding). Swept all PAYMENT.SALE.COMPLETED events Jul 29-Aug 12: 11 subscriptions charged; every one CANCELLED except the intentionally-active QA Platinum `I-D9S0UCEMDT03`. The month-old orphan `I-J5XBX9WEC98G` was cancelled in the owner's Aug 9 PayPal cleanup. `I-W4X6DVFPXV93` returns RESOURCE_NOT_FOUND -- it was a mistranscription, never existed. Two previously untracked IDs (`I-0KCMXAMW9NH7`, `I-E59URBUJ6482`) were the Aug 6 upgrade-test subs, cancelled same day. Known blind spot: a monthly sub that last charged BEFORE Jul 29 would not appear in the event window -- closed by the owner checking the buyer account's Payments -> Manage automatic payments -> Show active list: if it shows only the one Test Store entry ($94.50/day), the audit is complete with zero orphans. Probe subs I-9NFV9F997EYU / I-3S7W7426P2MB are APPROVAL_PENDING, never charge, ignore. |
...
| 404 | QA restart **day 4**: cancel + delete bob3test3 | Two-step finish: (1) owner clicks **Cancel Subscription** on the Billing page -- this exercises the agent-facing cancel button, the one path this restart has not touched (deletion-cancel is proven 2x, supersede-cancel 2x); verify `I-D9S0UCEMDT03` CANCELLED at PayPal. (2) Delete bob3test3 from SuperAdmin -- takes the spurious invoices 000011 test data with it. Then confirm zero events/charges the following day (trivially expected: Platinum was deferred to Sep 11 anyway). |
