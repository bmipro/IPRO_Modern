...
{
    public const int PollResultsMinResponses = 10;
    ...
    // 450: the same aggregation feeds the PollVote block's result view, so it lives in one place.
    public static async Task<PollResultsBlockData> AggregateAsync(IPRODbContext db, PollSurvey survey)
    {
    ...
    public static async Task<Dictionary<int, PollResultsBlockData>> BuildAsync(IPRODbContext db, int agentUserId, WebsitePage? currentPage, bool isOwnerPreview = false)
    {
        ...
        foreach (var block in pollBlocks)
        {
            ...
            var data = await AggregateAsync(db, survey);
