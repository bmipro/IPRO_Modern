...
public class PublicWebsiteController : Controller
{
    ...
    private async Task<IActionResult> BuildWebsiteViewAsync(IPRO.Entities.AgentWebsite website, string? slug)
    {
        ...
        var pollResultsByBlockId = await BuildPollResultsByBlockIdAsync(website.AgentUserId, currentPage);
    ...
    private async Task<Dictionary<int, PollResultsBlockData>> BuildPollResultsByBlockIdAsync(int agentUserId, WebsitePage? currentPage)
    {
        ...
        foreach (var block in pollBlocks)
        {
            ...
            var survey = await _db.PollSurveys.FirstOrDefaultAsync(s => s.Id == settings.PollSurveyId && s.AgentUserId == agentUserId);
            ...
            var recipientIds = await _db.PollRecipients.Where(r => r.PollSurveyId == survey.Id).Select(r => r.Id).ToListAsync();
            var counts = await _db.PollAnswers
